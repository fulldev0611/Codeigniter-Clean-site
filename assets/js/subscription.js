(function($) {
	...
	$( document ).ready(function() {
		...
		$(".pay_by_stripe").on("click", function() {
			...
			$.ajax({
                url: base_url + 'create-stripe-employee-subscription',
                type: 'post',
                dataType: 'json',
		...
		$(".pay_by_paypal").on("click", function() {
			...
			var listId = $(this).attr('data-list-id');
			$.ajax({
							url: 'otp-verification-subscriptions/start-paypal-pro-subscription',
							type: 'post',
							dataType: 'json',
							data: {csrf_token_name: csrf_token, subscription_id: listId},
							success: function (response) {
								if(response != null) {
									...
									if(subscription_id && plan_id !== null)
										window.location.href = 'otp-verification-paypal-pro-subscription/'+subscription_id+"/"+plan_id;
								}
							},
							error: function (error) {
									hideLoader();
					...
					"handler": function (response){
						  $.ajax({
							...
							success: function (msg) {						
							  window.location.href = base_url+'employee-subscription';
							}
						});
					},
					"theme": {
