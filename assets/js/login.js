(function($) {
  ...
  $(document).ready(function() {
    ...
    var email_valid_chk = false;    // Leo

    $('.upload_file').on('click', function() {
      $("#upload_type").val($(this).attr('rel'));
      if ($("#" + $(this).attr('rel')).val() == '') {
        alert("Please select " + $(this).data('title') + ' file');
        return false;
      } else {
        $("#" + $("#upload_type").val() + "_progress").html('Please wait');
        var formData = new FormData(document.getElementById('new_third_page'));
        formData.append('csrf_token_name', csrf_token);
        $.ajax({
            method: "POST",
            contentType: false,
            processData: false,
            url: base_url + "user/login/uploaddoc",
            data: formData
          })
          .done(function(msg) {
            $("#" + $("#upload_type").val() + "_progress").html('');
            if (msg == 'success') {
              $("#" + $("#upload_type").val() + "_progress").html('<img height="22" src="' + base_url + '/assets/img/check.png">');
              $("#" + $("#upload_type").val() + "_done").val(1);
            } else {
              $("#" + $("#upload_type").val() + "_progress").html('!Error. Please try again.');
              $("#" + $("#upload_type").val() + "_done").val('');
            }
          });
      }
    });
    ...
    function checkEmailId() {
      var emailid = $(".login_email").val(); //alert(emailid);
      // Leo: mail check
      if (emailid.length == 0) {
        email_valid_chk = false;
        clearAllErrors();
        $('#mailid_error').text('Please enter your Email ...').css('color', 'red');
        return;
      }
      // Leo: valid email check
      var mailformat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
      if(!emailid.match(mailformat))
      {
        email_valid_chk = false;
        clearAllErrors();
        $('#mailid_error').text('Please enter valid Email ID ...').css('color', 'red');
        return;
      }
    ...
    $('#emaillogin_finals').on('click', function() {
      check_emaillogin_otp();
    });
          ...
          $.ajax({
            ...
            success: function(res) {
              ...
              if (res.data == 1) {
                ...
                clearAllErrors();
                $('#mailid_error').show();
                $('#mailid_error').text('Email is not registered or deactivated ...').css('color', 'red');
                $('#emaillogin_submit').attr('disabled', true);
              }
          ...
          $.ajax({
            ...
            success: function(res) {
              ...
              if (res.data == 1) {
                ...
                $('#login_mode1').val(res.mode);
                $('#mailid_error').show();
                $('#mailid_error').text('Email not registered or Deactivated...').css('color', 'red');
                $('#login_submit').attr('disabled', true);
    ...
    function forgotpwd_function() {
      ...
      $("#flash_error_message1").hide();
      if (email.length == 0) {
        $('#mailid_error').text('Please Enter Email ...').css('color', 'red');
        $("#login_email").focus();
        return;
      }
      var csrf_token = $('#login_csrf').val();
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          if (data == 1) {
            ...
            $("#info_res").html("Reset Link Sent To Your Email ...!").css("color", "green");
            toaster_msg("info", "We have sent Reset Link To Your Email.")
            //window.location = base_url+'admin';
          }
          else if (data == 2) {
            // $("#err_respwd").html("This email is not registered or deactivated ...!").css("color", "red");
            toaster_msg("error","The email you entered is not registered or deactivated !");
          } 
          else {
            //location.reload();
            console.log(data);
            toaster_msg("error","Something went wrong. Failure email sending !");
    ...
    function check_emaillogin_otp() {
      ...
      if (email.trim() == "" && login_password.trim() == "") {
        clearAllErrors();
        $("#otp_error_msg_login").text("Please enter your login details ...").css('color','red');
        return;
      }
      checkEmailId();
      if (!email_valid_chk) {
      ...
      if (login_password.trim() == "") {
        clearAllErrors();
        $("#otp_error_msg_login").text("Please enter your password ...").css('color','red');
        return;
      }
      var mode = $("#login_mode").val();
      var csrf_token = $('#login_csrf').val();
      if (mode == 1) { //provider
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.response == 'ok') {
              ...
              try {
                if (history_uri && history_uri.trim() != "") {
                  window.location.href = base_url + history_uri;
                } else {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.response == 'ok') {
              try {
                // added maksimU : for employee
                switch( data.login_data.data.you_are_appling_as)
                {
                  case 10:                    
                  ...
                  case 8:
                    ...
                    return;break;
                  case 6:
                    window.location.href = base_url + 'soletrader-dashboard';
                    return;break;
                  default: break;
                }
                // <---
            ...
            else if (data.response == "two-factor-auth") {
              window.location.href = base_url + "login-2fa";
            } 
            else if (data.response == 'error') {
              login_button_unloading();
              $('#otp_error_msg_login').show();
    ...
    $('#new_third_page').bootstrapValidator({
      ...
      if ($("#id_proof_done").val() != 1) {
        alert("You have to upload valid id proof first in order to register to the website.");
    ...
    $('#new_third_page_user').bootstrapValidator({
      ...
      fields: {
        ...
        you_are_appling_as: {
          validators: {
            notEmpty: {
              message: 'Please Select what you are applying as ..'
            }
          }
        },
        house_name: {
      ...
      var you_are_appling_as = $('#are_appling_as').val();
      var house_name = $('#h_name').val();
      var house_number = $('#h_number').val();
      var street_address = $('#st_address').val();
      var city = $('#c_city').val();
      var province = $('#p_province').val();
      var postal_code2 = $('#p_postal_code').val();
      ...
      if (user_logintype == 'email') {
        $.ajax({
          ...
          success: function(data) {
            ...
            if (obj.response == 'ok') {
              //window.location.reload();
              window.location = base_url+"otp-verification";
            } else {
              console.log(obj.response);
              swal({
                title: "Something Went Wrong",
  ...
  function login_button_loading() {
    var $this = $('#emaillogin_finals');
  ...
  function login_button_unloading() {
    var $this = $('#emaillogin_finals');
    $this.html($this.data('original-text')).prop('disabled', false);
  }
})(jQuery);