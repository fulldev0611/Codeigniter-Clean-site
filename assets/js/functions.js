(function($) {
  ...
  $(document).ready(function() {
    ...
    $('#contact_form').bootstrapValidator({
      ...
      // add maksimU : for contact(employee, organization, staff) 
      var regurl = 'user/contact/insert_contact';
      var redirect = 'contact';
      if($('#contactto').val()=='admin') {
        regurl = 'employee/insert_contact_admin';
        redirect = 'employee-dashboard';
      }
      else if($('#contactto').val()=='user') {
        regurl = 'employee/insert_contact_user';
        redirect = 'employee-dashboard';
      } else if($('#contactto').val()=='organization_to_admin') {
        regurl = 'organization/insert_contact_admin';
        redirect = 'organization-dashboard';
      } else if($('#contactto').val()=='organization_to_user') {
        regurl = 'organization/insert_contact_user';
        redirect = 'organization-dashboard';
      } else if($('#contactto').val()=='staff_to_admin') {
        regurl = 'staff/insert_contact_admin';
        redirect = 'staff-dashboard';
      } else if($('#contactto').val()=='staff_to_user') {
        regurl = 'staff/insert_contact_user';
        redirect = 'staff-dashboard';
      } else if($('#contactto').val()=='partner_to_admin') {
        regurl = 'partner/insert_contact_admin';
        redirect = 'partner-dashboard';
      } else if($('#contactto').val()=='partner_to_user') {
        regurl = 'partner/insert_contact_user';
        redirect = 'partner-dashboard';
      }
      ...
      $.ajax({
        type: 'POST',
        url: base_url + regurl,                           // <-- update maksimU
        data: {
          userId: userId,                                 // <-- add maksimU
          name: name,
          email: email,
        ...
        success: function(response) {
          ...
          if (response == 1) {
            swal({
              ...
              window.location.href = base_url + redirect; // <-- update maksimU
            });
    ...
    // @Leo: post service click event 
    $('#post-service').on('click', function() {
      $.post(base_url + 'user/service/checkpost', {
        csrf_token_name: csrf_token
      }, function(response) {
        ...
        switch (result.notice) {
          case "OK":
            window.location.href = base_url + "add-service";
            break;
          case "GET_PRO_SUBSCRIPION":
            get_pro_subscription();
            break;
          case "GET_PRO_AVAILABILITY":
            get_pro_availabilty();
            break;
          case "FULL_POSTED":
            swal({
              title: "Service Post Warning!",
              text: "You can post " + result.max + " services at the most. You posted " + result.total + " services already.",
              icon: "warning",
              button: "okay",
              closeOnEsc: false,
              closeOnClickOutside: false
    ...
    $('.search_delivery').on('click', function() {
      $('#search_delivery').submit();
    });
    ...
    $.ajax({
      ...
      beforeSend: function() {
        ...
        $('.category-list').find("option:eq(0)").html("Please wait..");

      },
      success: function(data) {
        $('#categorys').find("option:eq(0)").html("Select Category");
        $('.category-list').find("option:eq(0)").html("Select Category");
  ...
  // --> add maksimU : for Video chat
  setInterval( function() {
    var getcall_url = 'ajax/get_videocall_tome';
    var redirect_url = 'employee-chat/avcall';
    $.ajax({
      type: 'POST',
      url: base_url + getcall_url,                          
      data: {
        csrf_token_name: csrf_token,
      },
      success: function(response_raw) {
        ...
        if (response.result == 1) {
          ...
          swal({
            title: "Video Calling to you!",
            text: "Hello! "+response.roominfo.name + ' ' + response.roominfo.l_name +" is calling you via Video Chating.",
            icon: "success",
            button: "okay",
            closeOnEsc: true,
            closeOnClickOutside: true
          }).then(function() {
            window.location.href = base_url + redirect_url+  '?roomId=' + response.roominfo.room_identifier; // <-- update maksimU
          });
        } else {
          $("#flash_error_message1").show();
          $('#flash_error_message1').append('Error!');
          return false;
        }
      }
    });
