...
$(".img-upload-input-bs").on('change',function(event) {
    ...
    input_target = $(this).attr('input-target');
...
$(".img-upload-btn-bs").on('click',function(event) {
    ...
    cropper.croppie('result',{
        ...
        {
            $.ajax({
                data:{"image":response,"name":name,"csrf_token_name":csrf_token},
                url:passurl,
                type:'POST'
            }).done(function(data)
              {
                ...
                if(target != undefined && data["response"])
                {
                    ...
                    if (data["image_path"]) {
                        $(input_target).val(data["image_path"]);
                    }
                    $(target).attr('src', data["response"]);
                }
...
$(".img-remove-btn-bs").on('click', function(event) {
    Filter.original=undefined;
    Filter.previous=undefined;
    $(modal).modal('hide');
    if(img_input!=undefined)
