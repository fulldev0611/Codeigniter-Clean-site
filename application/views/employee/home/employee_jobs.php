<div class="content">
    <div class="container">
        <div class="row">
            ...
            $this->load->view('user/home/employee_sidemenu'); ?>
                                    ...
                                    <div class="booking-action">
                                        ...
                                        <?php if ($bookings['status'] == 2) {
                                            ...
                                            <a href="javascript:;" class="btn btn-sm bg-danger-light myCancel" data-toggle="modal" data-target="#myCancel" data-id="<?php echo $bookings['id'] ?>" data-providerid="<?php echo $bookings['provider_id'] ?>" data-userid="<?php echo $bookings['user_id'] ?>" data-serviceid="<?php echo $bookings['service_id'] ?>">  
                                                <i class="fa fa-times"></i> <?php echo (!empty($user_language[$user_selected]['lg_cancel_service'])) ? $user_language[$user_selected]['lg_cancel_service'] : $default_language['en']['lg_cancel_service'];//start_on
                                                 ?>
                                            </a>  
        <?php  

           $query = $this->db->query('SELECT * FROM employee_job WHERE service_id='.$bookings['service_id']);
         ...
         $query = $this->db->query('SELECT * FROM services WHERE id='.$bookings['service_id']);

           $result = $query->result(); 
        
           $row1 = $result['0'];
           
           $serviceamounttype = $row1->serviceamounttype;
           //echo $serviceamounttype;
           if ($serviceamounttype=='Hourly') {
           ?>                 
                                          <!-- <a href="<?php base_url(); ?>emp_job_book1?bookings_id=<?php echo $bookings['id']; ?>&start_on=1" class="btn btn-sm bg-info-light">
                                                <i class="fa fa-check"></i>Start Time
                                          </a> --> 
                                          <?php if ($bookings['start_time']=="") { ?>
                                             <form action="<?php base_url(); ?>emp_job_book1?bookings_id=<?php echo $bookings['id']; ?>" method="post" class="form-inline">
                                            <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                                            <div class="row">
                                            <div class="form-group">
                                              <input type="text" class="form-control text-dark border-dark" placeholder="Add Start Time" name="datetime" style="width:177px;">
                                            </div>
                                        <div>
                                            <input type="submit" style="height:47px;" value="Add" class="btn btn-primary">
                                        </div>
                                    </div>
                                          </form>
                                         <?php }else{ ?>
                                         <?php echo $bookings['start_time']; ?>
                                         <?php } ?>
                                           
                                     
                                          <!-- <a href="<?php base_url(); ?>emp_job_book2?bookings_id=<?php echo $bookings['id']; ?>&end_on=1" class="btn btn-sm bg-info-light">
                                                <i class="fa fa-times"></i>End Time
                                          </a> --> 
                                        ...
                                        <!-- <a href="javascript:;" class="btn btn-sm bg-success-light update_pro_booking_status"  data-id="<?= $bookings['id']; ?>" data-status="3" data-rowid="<?= $pending; ?>" data-review="2">
                                                <i class="fa fa-check"></i> <?php echo (!empty($user_language[$user_selected]['lg_complete_res_user'])) ? $user_language[$user_selected]['lg_complete_res_user'] : $default_language['en']['lg_complete_res_user']; ?>
                                            </a> -->
                                       ...
                                       <?php } ?>
                                        ...
                                        <?php } elseif ($bookings['status'] == 1) {
                                            ...
                                            <a href="javascript:;" class="btn btn-sm bg-success-light update_pro_booking_status"  data-id="<?= $bookings['id']; ?>" data-status="2" data-rowid="<?= $pending; ?>" data-review="2" >
                                                <i class="fa fa-check"></i> <?php echo (!empty($user_language[$user_selected]['lg_user_res_accept'])) ? $user_language[$user_selected]['lg_user_res_accept'] : $default_language['en']['lg_user_res_accept']; ?>
                                            </a>
                                            <a href="javascript:;" class="btn btn-sm bg-danger-light myCancel" data-toggle="modal" data-target="#myCancel" data-id="<?php echo $bookings['id'] ?>" data-providerid="<?php echo $bookings['provider_id'] ?>" data-userid="<?php echo $bookings['user_id'] ?>" data-serviceid="<?php echo $bookings['service_id'] ?>">  
                                                <i class="fa fa-times"></i> <?php echo (!empty($user_language[$user_selected]['lg_cancel_service'])) ? $user_language[$user_selected]['lg_cancel_service'] : $default_language['en']['lg_cancel_service']; ?>
                                            </a>
