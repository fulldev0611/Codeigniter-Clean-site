					...
					<div class="widget">
						...
						<div class="row">
							...
							<div class="form-group col-xl-6 required">
								...
								<input type="text" class="form-control birthdaypicker" autocomplete="off" name="dob" value="<?php echo (!empty($get_details['dob']))?date('d-m-Y',strtotime($get_details['dob'])):'';?>">
							</div>

							<div class="form-group col-xl-6">
								<label class="mr-sm-2">Website Link</label>
								<input class="form-control" type="text"  value="<?php echo $get_details['site_link']?>" name="site_link" placeholder="https://example.com">
							</div>
							...
							<div class="form-group col-xl-4 required">
								<label class="control-label mr-sm-2"><?php echo (!empty($user_language[$user_selected]['lg_What_Service_do_you_Provide'])) ? $user_language[$user_selected]['lg_What_Service_do_you_Provide'] : $default_language['en']['lg_What_Service_do_you_Provide']; ?></label>
							...
							<div class="form-group col-xl-4 required">
								<label class="control-label mr-sm-2"><?php echo (!empty($user_language[$user_selected]['lg_Sub_Category'])) ? $user_language[$user_selected]['lg_Sub_Category'] : $default_language['en']['lg_Sub_Category']; ?></label>
								<select class="form-control select provider_subcategory" title="Sub Category" name="subcategorys" id="subcategorys_old" data-live-search="true" >
							...
							<div class="form-group col-xl-4 required">
								<label class="control-label mr-sm-2">currency</label>
								<select class="form-control select provider_subcategory" title="Currency Code" name="currency_code" id="subcategorys_old" data-live-search="true" required="">
									<?php foreach ($currency as $currency_row) {
									 if($get_details['currency_codecategory']==$sub_currency_code['category']){ ?>
									<option value="<?php echo $currency_row['currency_code']?>"  <?php 
									if ($currency_row['currency_code'] == $get_details['currency_code']) { ?> selected = "selected" <?php }   ?>><?php echo $currency_row['currency_code']?>
									</option>
									<?php } } ?>
								</select>
							</div>
...
<script src="<?php echo base_url(); ?>assets/js/provider_settings.js?v1.03"></script>