...
//                                           
                                    ...
                                    <div class="booking-action">
                                        ...
                                        <?php if ($bookings['status'] == 2) { ?>
                                            <a href="<?php echo base_url() ?>partner-chat/partner-order-new-chat?book_id=<?php echo $bookings['id'] ?>" class="btn btn-sm bg-info-light">
                                            ...
                                            <a onclick="completedRequestBooking(this);" class="btn btn-sm bg-success-light"  data-id="<?= $bookings['id']; ?>" data-status="3" data-rowid="" data-review="">
...
<script type="text/javascript">
        ...
        $.confirm({
            ...
            buttons: {
                confirm: function() {                    
                    ...
                    $.ajax({
                        url: base_url + "update_status_by_partner",
                        data: {
                            'booking_id': bookid,
                            'status': status,
                            'review': review,
                        ...
                        success: function(response) {
                            ...
                            if (response == '1') { //success updated
                                swal({
                                    ...
                                    // window.location.reload();                                    
                                    window.location.href = "<?=base_url()?>partner-chat/partner-order-new-chat?book_id="+booking_id;
                                });
                            }
                        }
                    })
    ...
    function completedRequestBooking(obj){
        var booking_id = $(obj).attr("data-id");
        var status = '<?php echo BS_COMPLETED_PROVIDER; ?>';
        var review = "";
        var token = $('#csrf_token').val();
        $.confirm({
            title: 'Confirmations..!',
