					...
					<div class="tab-pane show active" id="user_profile_settings" >
                        ...
                        <table class="table table-bordered" id="myTable">
                            ...
                            <tbody id="tBody">
															...
															?>
																<tr value="<?php echo $staff['user_id'] ?>" id="table_id">
																	<td style="padding: 0%; text-align: center">
																		<?php if($user['profile_img']) { ?>
																			<img class="profile-img" src="<?php echo $user['profile_img'] ?>" alt="" />
																		<?php 
																		...
																		?>
																		<p style="margin: 0; padding-bottom: 3%" ><?php echo $staff['name'] ?></p>
																	</td>
																	<td value="<?php echo $staff['user_id'] ?>" class="monaday" id="<?php echo $staff['user_id'] ?>monday"></td>
																	<td value="<?php echo $staff['user_id'] ?>" class="tuesday" id="<?php echo $staff['user_id'] ?>tuesday"></td>
																	<td value="<?php echo $staff['user_id'] ?>" class="wednsday" id="<?php echo $staff['user_id'] ?>wednsday"></td>
																	<td value="<?php echo $staff['user_id'] ?>" class="thursday" id="<?php echo $staff['user_id'] ?>thursday"></td>
																	<td value="<?php echo $staff['user_id'] ?>" class="friday" id="<?php echo $staff['user_id'] ?>friday"></td>
																	<td value="<?php echo $staff['user_id'] ?>" class="saturday" id="<?php echo $staff['user_id'] ?>saturday"></td>
																	<td value="<?php echo $staff['user_id'] ?>" class="sunday" id="<?php echo $staff['user_id'] ?>sunday"></td>
																</tr>
															<?php 
																}
															?>
				...
				<div class="account-content">
					<div class="account-box">
						<div class="login-right">
							...
							<form method="post" action="organization-job-scheduling">
										...
										<select class="form-control select" id="user_id" name="user_id" required>
											...
											<?php 
												if(!empty($staffs)) {
													foreach($staffs as $staff) {
											?>
												<option name="<?php echo $staff['user_id'] ?>" value="<?php echo $staff['user_id'] ?>"><?php echo $staff['name'] ?></option>
											<?php 
													}
												}
											?>
...
<div class="modal account-modal fade multi-step" id="add-detail-shift<?php echo $staff['user_id'].$weekDates[$i]?>" data-keyboard="false" data-backdrop="static">
	<div class=" modal-lg modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header p-0 border-0" style="justify-content: center; padding-top: 5% !important; border-bottom: solid 1px #cccccc !important; padding-bottom: 5% !important">
        <h2 style="color: #6c2c78">
						...
						<div class="login-right">
							...
							<form method="post" action="user-job-scheduling">
								...
								<div style="display: flex">
									...
									<div style="width: 70%">
										<input class="form-control" id="<?php echo 'shift_date'.$staff['user_id'].$weekDates[$i] ?>" name="date" type="date"></input>
									</div>
								</div>
								<div style="display: flex; padding-top: 3%">
									<div class="from-to" style="width: 30%">
									...
									<div class="form-group" style="width: 30%; display: flex">
										<input class="form-control" type="time" id="<?php echo 'shift_start'.$staff['user_id'].$weekDates[$i]?>" name="shift_start" min="09:00" max="18:00" required></input>
									</div>
									<div class="from-to" style="padding-left: 3%; padding-right: 3%; width: 10%"><h3> End</h3></div>
									<div class="form-group" style="width: 30%; display: flex">
										<input class="form-control" type="time" id="<?php echo 'shift_end'.$staff['user_id'].$weekDates[$i]?>" name="shift_end" min="09:00" max="18:00" required></input>
									</div>
								</div>
								<hr></hr>
								<div style="display: flex;">
									...
									<div style="width: 70%">
										<div class="form-group">
											<input class="form-control" type="text" name="shift_title" id="<?php echo 'shift_title'.$staff['user_id'].$weekDates[$i] ?>" placeholder="Type Here" required>
								...
								<div style="display: flex">
									...
									<div style="width: 70%">
										<div class="form-group">
											<input class="form-control" type="text" name="shift_location" id="<?php echo 'shift_location'.$staff['user_id'].$weekDates[$i] ?>" placeholder="Type Location" required>
										</div>
									</div>
								</div>
								<div class="form-group" style="padding-top: 5%; display: flex">
									...
									<div style="width: 70%">
										<select class="form-control select" id="<?php echo 'user_id'?>" name="user_id" required>
											<option value="">Select Employee</option>
											<option name="<?php echo $staff['user_id'] ?>" id="<?php echo 'user_id'.$staff['user_id']?>" value="<?php echo $staff['user_id'] ?>"><?php echo $staff['name'] ?></option>
										</select>
									</div>
								</div>
								<div class="form-group" style="padding-top: 5%; display: flex">
									...
									<div style="width: 70%">
										<label>Category <span class="text-danger">*</span></label>
										<select class="form-control select" id="<?php echo 'category'.$staff['user_id'] ?>" name="category" required>
											<option value="">Select Category</option>
											<?php 
												if(!empty($lists)) {
													foreach($lists as $category) {
										...
										<div class="form-group" style="padding-top: 5%">
											<label>Services<span class="text-danger">*</span></label>
											<select class="form-control select" title="Select Services" name="<?php echo 'subcategory'.$staff['user_id'] ?>" id="<?php echo 'subcategory'.$user['id'] ?>" required></select>
										</div>
									</div>
								</div>
								<div style="display: flex">
									...
									<div style="width: 70%">
										<textarea class="form-control" id="<?php echo 'schedule_note'.$staff['user_id'].$weekDates[$i] ?>" name="schedule_note" placeholder="Attach a note to your request">
										</textarea>
									</div>
								</div>
								<div style="padding: 5%">
									<button type="submit" id="<?php echo 'shift_publish'.$staff['user_id'] ?>" class="login-btn btn" style="border-radius: 50px">Publish</button>
								</div>
							</form>
						</div>
					</div>
...
$(document).ready(function(){
	...
	$('#category').click(function() {
		...
		$.ajax({
			type: "POST",
			url: base_url + "organization/service/get_subcategory",
			data: {
				id: $(this).val(),
	...
	function populateDates() {
		...
		$.ajax({
				type: "POST",
				url: "../../organization/dashboard/shift_get_data",
				data: {
					weekDates: postDates,
					csrf_token_name: csrf_token
				},
				success: function(response) {
					...
					if(temp[0].length > 0){
						for(var i=0; i<temp[0].length; i++){
							...
							var mon = `<div id="${temp[0][i].user_id}mon" onclick="detailFun(${temp[0][i].user_id})" data-toggle="modal" data-target="#add-detail-shift${temp[0][i].user_id}mon" class='schedule-detail'>${temp[0][i].schedule_job}</br>${temp[0][i].shift_start} - ${temp[0][i].shift_end}</div>`;
					...
					if(temp[1].length > 0){
						for(var i=0; i<temp[1].length; i++){
							...
							var tue = `<div id="${temp[1][i].user_id}tue" onclick="detailFun(${temp[1][i].user_id})" data-toggle="modal" data-target="#add-detail-shift${temp[1][i].user_id}tue" class='schedule-detail'>${temp[1][i].schedule_job}</br>${temp[1][i].shift_start} - ${temp[1][i].shift_end}</div>`;
					...
					if(temp[2].length > 0){
						for(var i=0; i<temp[2].length; i++){
							$('#' + temp[2][i].user_id + 'wed').attr("style", "display: none");
							var wed = `<div id="${temp[2][i].user_id}wed" onclick="detailFun(${temp[2][i].user_id})" data-toggle="modal" data-target="#add-detail-shift${temp[2][i].user_id}wed" class='schedule-detail'>${temp[2][i].schedule_job}</br>${temp[2][i].shift_start} - ${temp[2][i].shift_end}</div>`;
					...
					if(temp[3].length > 0){
						for(var i=0; i<temp[3].length; i++){
							...
							var thur = `<div id="${temp[3][i].user_id}thu" onclick="detailFun(${temp[3][i].user_id})" data-toggle="modal" data-target="#add-detail-shift${temp[3][i].user_id}thu" class='schedule-detail'>${temp[3][i].schedule_job}</br>${temp[3][i].shift_start} - ${temp[3][i].shift_end}</div>`;
					...
					if(temp[4].length > 0){
						for(var i=0; i<temp[4].length; i++){
							$('#' + temp[4][i].user_id + 'fri').attr("style", "display: none");
							var fri = `<div id="${temp[4][i].user_id}fri" onclick="detailFun(${temp[4][i].user_id})" data-toggle="modal" data-target="#add-detail-shift${temp[4][i].user_id}fri" class='schedule-detail'>${temp[4][i].schedule_job}</br>${temp[4][i].shift_start} - ${temp[4][i].shift_end}</div>`;
					...
					if(temp[5].length > 0){
						for(var i=0; i<temp[5].length; i++){
							$('#' + temp[5][i].user_id + 'sat').attr("style", "display: none");
							var sat = `<div id="${temp[5][i].user_id}sat" onclick="detailFun(${temp[5][i].user_id})" data-toggle="modal" data-target="#add-detail-shift${temp[5][i].user_id}sat" class='schedule-detail'>${temp[5][i].schedule_job}</br>${temp[5][i].shift_start} - ${temp[5][i].shift_end}</div>`;
					...
					if(temp[6].length > 0){
						for(var i=0; i<temp[6].length; i++){
							$('#' + temp[6][i].user_id + 'sun').attr("style", "display: none");
							var sun = `<div id="${temp[6][i].user_id}sun" onclick="detailFun(${temp[6][i].user_id})" data-toggle="modal" data-target="#add-detail-shift${temp[6][i].user_id}sun" class='schedule-detail'>${temp[6][i].schedule_job}</br>${temp[6][i].shift_start} - ${temp[6][i].shift_end}</div>`;
	...
	$('#shift_publish').click(function() {
		...
		$.ajax({
				url: '../../organization/dashboard/shift_add_data',
				type: 'POST',
				data: {
...
m = n.getMonth() + 1;
d = n.getDate();
document.getElementById("date").innerHTML =  weekDays[n.getDay()].slice(0,3) + ", " + months[n.getMonth()].slice(0,3) + " "+ d + ", " + y;

function detailFun(detail) {

	$('#category' + detail).click(function() {
		var csrf_token = $('#csrf_token').val();
console.log(csrf_token, '---here');
	...
	$('#shift_publish' + detail).click(function() {
		...
		$.ajax({
				url: '../../organization/dashboard/shift_update_data',
				type: 'POST',
				data: {
