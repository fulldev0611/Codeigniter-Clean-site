					...
					<div class="tab-pane show active" id="user_profile_settings" >
                        ...
                        <table class="table" id="myTable">
													...
													<tbody>
														...
														?>
															<tr style="cursor: pointer; border-bottom: solid 1px #dee2e6" onclick="modalFunction(<?php echo $staff['user_id'] ?>)" data-toggle="modal" id="<?php echo $staff['user_id'] ?>" data-target="<?php echo '#shift'.$staff['user_id']?>" >
																<td style="padding: 0%; text-align: center">
																	<?php if($staff['profile_img']) { ?>
																		<img class="profile-img" src="<?php echo $staff['profile_img'] ?>" alt="" />
																	<?php 
																			}
																		else {
																	?>
																	...
																	<?php 
																		...
																		foreach($clocked_today as $clocked) {
																			if($staff['user_id'] === $clocked['user_id']) {
																	?>
...
<?php 
	foreach($staffs as $staff) {
?>
<div class="modal account-modal fade multi-step" id="<?php echo 'shift'.$staff['user_id'] ?>" value="<?php echo $staff['user_id'] ?>" data-keyboard="false" data-backdrop="static">
		...
		<div class="modal-content">
			<div class="modal-header p-0 border-0" style="justify-content: left; padding-top: 5% !important; border-bottom: solid 1px #cccccc !important; padding-bottom: 5% !important">
				<div style="display: flex; padding-left: 5%">
					<?php if($staff['profile_img']) { ?>
						<img class="profile-img-modal" src="<?php echo $staff['profile_img'] ?>" alt="" />
					<?php 
							}
						else {
					?>
					...
					<div class="row" style="color: #6c2c78; justify-content: center; align-items: center">
						<form action="employee/timesheet" method="POST">
							...
							<div class="col-8">
								...
								<input type="text" style="border-radius: 50px; text-align: center" class="form-control mb-2 mr-sm-2 col-4 weekcommencing" name="<?php echo 'datePicker'.$staff['user_id'] ?>" id="<?php echo 'datePicker'.$staff['user_id'] ?>" placeholder="week commencing">
							</div>
				...
				<button type="button" class="close" data-dismiss="modal" onclick="closeFunc(<?php echo $staff['user_id'] ?>)" id="close<?php echo 'close'.$staff['user_id'] ?>" aria-label="Close">
						...
						<div class="login-right">
							<div class="table-responsive">
								<table class="table" id="myTable">
									...
									<tbody id="<?php echo 'tBody'.$staff['user_id'] ?>">
										<tr>
							...
							<div style="padding: 5%; text-align: right">
								<button class="login-btn btn" onclick="saveChanages(<?php echo $staff['user_id'] ?>)" id="<?php echo 'save-button'.$staff['user_id'] ?>" style="display: inline-flex;"> Save Changes </button>
							</div>
						</div>
					</div>
				</div>
...
function modalFunction(userID) {
    ...
    function populateDates() {
			...
			$.ajax({
					type: "POST",
					url: "../../organization/dashboard/shift_detail_get_data",
					data: {
						weekDates: weekDates,
						postDates: postDates,
						userID: userID,
...
function saveChanages(userID) {
	...
	for(let i=0; i<monStartWeekDays.length; i++) {
		$.ajax({
				url: '../../organization/dashboard/shift_detail_add_data',
				type: 'POST',
				data: {
	...
	window.location.href="organization-time-clock";
}
