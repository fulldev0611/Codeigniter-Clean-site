...
//                                           
                                            ...
                                            <ul class="booking-details">
                                                ...
                                                <?php if ( !empty($bookings['staffname']) && 
                                                            ($bookings['status'] == BS_INPROGRESS || $bookings['status'] == BS_COMPLETED || $bookings['status'] == BS_ACCEPTED) )
                                                {
                                                    $badge = '';
                                                    $class = '';
                                                    if ($bookings['staffstatus'] == ORG_BS_PENDING) {
                                                        $badge = 'Pending';
                                                        $class = 'bg-warning';
                                                    }
                                                    if ($bookings['staffstatus'] == ORG_BS_INPROGRESS) {
                                                        $badge = 'Inprogress';
                                                        $class = 'bg-primary';
                                                    }
                                                    
                                                    if ($bookings['staffstatus'] == ORG_BS_ACCEPTED) {
                                                        $badge = 'Accepted';
                                                        $class = 'bg-success';
                                                    }
                                                    if ($bookings['staffstatus'] == ORG_BS_REJECTED) {
                                                        $badge = 'Rejected';
                                                        $class = 'bg-danger';
                                                    }
                                                    if ($bookings['staffstatus'] == ORG_BS_COMPLETED) {
                                                        $badge = 'Completed';
                                                        $class = 'bg-success';
                                                    }
                                                    if ($bookings['staffstatus'] == ORG_BS_CANCELLED) {
                                                        $badge = 'Cancelled';
                                                        $class = 'bg-danger';
                                                    }
                                                    ?>                                                <li>
                                                ...
                                                <li>
                                                    <?php 
                                                        if (!empty($bookings['staffimg'])) {
                                                            $image = base_url() . $bookings['staffimg'];
                                                        } else {
                                                            $image = base_url() . 'assets/img/user.jpg';
                                                        }                                                    
                                                    ?>
                                                    ...
                                                    <div class="avatar avatar-xs mr-1">
                                                        <img class="avatar-img rounded-circle" alt="User Image" src="<?php echo $image; ?>">
                                                    </div> <?= !empty($bookings['staffname']) ? $bookings['staffname'] : '-'; ?>
                                                    <span class="badge badge-pill badge-prof <?php echo $class; ?>"><?= $badge; ?></span>
                                                </li>
                                                <li><span>Phone</span>  <?php echo $bookings['staffphone'] ?></li>
                                                    <?php 
                                                }
                                                ?>
                                    ...
                                    <div class="booking-action">
                                        ...
                                        <?php 
                                        if ($bookings['status'] == BS_PENDING) { ?>
                                            <a onclick="initAcceptData(this)" class="btn btn-sm bg-success-light myAccept" data-toggle="modal" data-target="#myAccept"  data-id="<?=$bookings['id'];?>" data-status="2" data-rowid="" >
                                                <i class="fa fa-check"></i>Accept</a>
                                        <?php } ?>
                                        <?php if ($bookings['status'] == BS_INPROGRESS) { ?>
                                            <?php if( empty($bookings['staffname'])) {
                                                ?>
                                                <a onclick="initAcceptData(this)" class="btn btn-sm bg-success-light myAccept" data-toggle="modal" data-target="#myAccept"  data-id="<?=$bookings['id'];?>" data-status="2" data-rowid="" >
                                                    <i class="fa fa-check"></i>Reassign Staff</a>
                                                <?php
                                            } ?>
                                            <a href="<?php echo base_url() ?>organization-chat/organization-order-new-chat?book_id=<?php echo $bookings['id'] ?>" class="btn btn-sm bg-info-light">
...
<!-- Accept Modal -->
<div class="modal fade" id="myAccept">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Accept and Assign Service to staff</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <select class="select2 form-control" id="staff_id" name="staff_id">
                        <?php 
                            foreach ($staffs as $staff) {
                        ?>
                                <option value="<?= $staff['id'] ?>"><?php echo $staff['name']; ?></option>
                        <?php
                            }
                        ?>
                    </select>
                ...
                <div class="text-center">                    
                    <button type="button" class="btn btn-theme py-2 px-4 text-white mx-auto" id="accept_booking" >Accept and Assign</button>
                </div>
            </div>
        </div>
    </div>
...
function initAcceptData(obj){
    $("#accept_booking_id").val('');
    var booking_id = $(obj).attr("data-id");
    $("#accept_booking_id").val(function() {
...
$('#accept_booking').on('click', function() {
    ...
    var booking_id = $("#accept_booking_id").val();
    if(booking_id>0 && staff_id>0){
        acceptBooking(booking_id, staff_id);
    }    
});

function acceptBooking(booking_id, staff_id){
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function() {                    
                ...
                $.ajax({
                    url: base_url + "update_status_by_organization",
                    data: {
                    ...
                    success: function(response) {
                        ...
                        if (response == '1') { //success updated
                            swal({
                                ...
                                window.location.reload();                                    
                                /*window.location.href = "<?=base_url()?>organization-chat/organization-order-new-chat?book_id="+booking_id;*/
                            });
                        }
                    }
                })
