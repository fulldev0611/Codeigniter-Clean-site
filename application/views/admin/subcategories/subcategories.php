...
<div class="page-wrapper">
	<div class="content container-fluid">
		...
		// repaired maksimU for error
								 $thumb_img = isset($rows['thumb_image'])?$rows['thumb_image']:'';
								echo'<tr>
								<td>'.$i++.'</td>
								<td>'.$rows["unique_id"].'</td>
								<td><img class="avatar-sm rounded mr-1" src="'.base_url().$thumb_img.'"> '.$rows['subcategory_name'].'</td>
		<!-- maksimU end -->								
								<td>'.$rows['category_name'].'</td>
								<td>'.$date.'</td>
								<td class="text-right"><a href="'.base_url().'edit-subcategory/'.$rows['id'].'" class="btn btn-sm bg-success-light mr-2"><i class="fa fa-edit mr-1"></i>Edit</a>
								<a href="javascript:void(0);" onClick="delete_subcategories($(this).attr(\'data-id\'));" class="on-default remove-row btn btn-sm bg-danger-light mr-2 delete_subcategories" id="Onremove_'.$rows['id'].'" data-id="'.$rows['id'].'"><i class="fa fa-trash-alt mr-1"></i> Delete</a>
								</td>
								</tr>';
