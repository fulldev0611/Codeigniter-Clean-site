...
<div class="page-wrapper">
	<div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row">
				<div class="col">
					<h3 class="page-title">User Login History</h3>
				</div>
		...
		<!-- Search Filter -->
		<form action="<?php echo base_url()?>user-login-history" method="post" id="filter_inputs">
							...
							<table class="table table-hover table-center mb-0 filter-data-table" >
								...
								if(!empty($list)) {
									...
									foreach ($list as $rows) {
										...
										if(!empty($rows['login_time'])){
											$time=date('Y-m-d H:i',strtotime($rows['login_time']));
										}else{
											$time='-';
										}
										...
										echo'<tr>
										<td>'.$i++.'</td>
										<td><img class="avatar-sm rounded mr-1" src="'.base_url().$rows['profile_img'].'"> '.$rows['user_name'].($rows['user_type']==1?' (provider)':'').'</td>
										<td>'.$rows['client_ip'].'</td>
										<td>'.$time.'</td>
										</tr>';
