...
/**
 * @author Leo SettingValue func upgrade
...
/**
 * @author Leo: show host seller site
 * @return Bool
*/
...
/**
 * @author Leo: show price
 * @return Bool
*/
if (!function_exists("show_price")) {
  // code...
  function show_price() {
    return settingValue("show_price")=="on"?true:false;
  }
}
...
if(!function_exists('getPopularServices')) {
  ...
  function getPopularServices($limit = 3) {
    ...
    return $ci->Service_model->getPopularServices(['limit'=>$limit]);
  }
...
/**
 * @author Leo: get popular delivery categories
 * @param limit number 
 * @return array
*/
if(!function_exists('getPopularDeliveryCategories')) {

  function getPopularDeliveryCategories($limit = 3) {
    $ci =& get_instance();
    $ci->load->model("DeliveryCategoriesModel");
    return $ci->DeliveryCategoriesModel->getPopularCategories($limit);
  }
...
if(!function_exists('paypalKeys')) {
  ...
  function paypalKeys($key = null) {
    ...
    if (is_array($data)) {
      $data['base_api_url'] = "https://api-m.paypal.com";
      if ($data['gateway_type'] == "sandbox") {
        $data['base_api_url'] = "https://api-m.sandbox.paypal.com";
      }
      if (!is_null($key)) {
        return $data[$key];
      }
...
if (!function_exists('image_resize')) {
  ...
  function image_resize($width = 0, $height = 0, $image_url, $filename, $path="") {
    ...
    $upload_path = "uploads".($path!=""?"/".$path:"");
    $image_url = $upload_path . "/" . $filename_without_extension . "_" . $width . "_" . $height . "." . $extension;
...
if (!function_exists('replace_specials')) {
  ...
  function replace_specials($str) {
    return str_replace(SEPCIAL_CHARACTERS, "-", $str);
  }
}

/**
 * @author Leo: Check Login Redirect 
 * @return array 
*/
if (!function_exists('checkLoginRedirect')) {
  ...
  function checkLoginRedirect() {
    ...
    if (empty($ci->session->userdata('id')))
    {
      ...
      $ci->session->set_flashdata('history_uri', $currentURI);
...
/**
 * @author Leo: Check If Not Provider
 * @return array 
*/
if (!function_exists('checkIfNotProvider')) {
  
  function checkIfNotProvider() {
    $ci =& get_instance();
    if (empty($ci->session->userdata('id')))
    {
      redirect(base_url());
    }
    if ($ci->session->userdata('usertype') == "provider") {
      redirect(base_url());
    }
  }
}
...
/**
 * @author Leo: Get User Details by token
 * @return array 
*/
if (!function_exists('getUserInfoByToken')) {
  ...
  function getUserInfoByToken($token) {
    $ci =& get_instance();
    $user_table=$ci->db->select('*')->
                    from('users')->
                    where('token',$token)->
                    get()->row();
    $provider_table=$ci->db->select('*')->
                    from('providers')->
                    where('token',$token)->
                    get()->row();
    if(!empty($user_table)){
...
/**
 * @author Leo: Check User Form and Profile Verify
 * @return array 
*/
if (!function_exists('checkUserFormAndProfile')) {
  
  function checkUserFormAndProfile() {
    $ci =& get_instance();
    $f_seg = $ci->uri->segment('1');
    $s_seg = $ci->uri->segment('2');
    if ($ci->input->server('REQUEST_METHOD') === 'POST' || $f_seg == "check" || $f_seg == "ajax") {
      // code...
      return;
    }
    // Check User Form and Profile Verify
    if ($ci->session->userdata('id') && $ci->session->userdata("usertype") == "user") {      // if logined. in case of user
      // code...
      if ($ci->session->userdata('fill_up_form') == 0) {    // if not fill up the form
          // code...
        if ($ci->uri->segment('1') != "user-form") {
          $ci->session->set_flashdata('popup_alert', [
            'title'=>"We need to know you ...",
            'text'=>"Please fill up and verify this form ...!",
            'icon'=>'info'
          ]);
          $ci->session->set_flashdata('info_message', "Please fill up and verify this Form!");
          redirect(base_url().'user-form');
        }
      }
      else if ($ci->session->userdata('fill_up_profile') == 0) {    // if not fill up profile
          // code...
        $applyingAs = $ci->session->userdata("you_are_appling_as");
        $ci->session->set_flashdata('popup_alert', [
          'title'=>"Please Verify Your Profile", 
          'text'=>"You need to fill up and verify Your Profile ...!",
          'icon'=>'info'
        ]);
        $ci->session->set_flashdata('info_message', "Please fill up and verify Your Profile!");

        if ($applyingAs == C_YOUARE_SOLETRADER) {
          // code...
          if ($ci->uri->segment('1') != "soletrader-settings") {
            redirect(base_url().'soletrader-settings');
          }
        }
        else if ($applyingAs == C_YOUARE_EMPLOYEE) {
          // code...
          if ($ci->uri->segment('1') != "employee-settings") {
            redirect(base_url().'employee-settings');
          }
        }
        else if ($applyingAs == C_YOUARE_SELF_EMPLOYED) {
          // code...
          if ($ci->uri->segment('1') != "self-employed-settings") {
            redirect(base_url().'self-employed-settings');
          }
        }
        else if ($applyingAs == C_YOUARE_ORGANIZATION) {
          // code...
          if ($ci->uri->segment('1') != "organization-settings") {
            redirect(base_url().'organization-settings');
          }
        }
        else if ($applyingAs == C_YOUARE_STAFF) {
          // code...
          if ($ci->uri->segment('1') != "staff-settings") {
            redirect(base_url().'staff-settings');
          }
        }
        else if ($applyingAs == C_YOUARE_PARTNER) {
          // code...
          if ($ci->uri->segment('1') != "partner-settings") {
            redirect(base_url().'partner-settings');
          }
        }
        else{
          if ($ci->uri->segment('1') != "user-settings") {
...
/**
 * @author Leo: generate Verification Code
 * @return string
*/
if (!function_exists('generateCode')) {
  
  function generateCode($length, $type = null)
  {
    $codeAlphabet = "";
    switch($type) {
      case 'digit':
    ...
    $code = "";
    for ($i = 0; $i < $length; $i++) {
        $code .= $codeAlphabet[crypto_rand_secure(0, $max - 1)];
    }
...
if (!function_exists('getStateList')) {
  ...
  function getStateList($countryId = null) {
    ...
    $stateList = $ci->db->get()->result_array();
...
if (!function_exists('getCityList')) {
  ...
  function getCityList($stateId = null) {
    $ci =& get_instance();
    $ci->db->select("*")->from('city');
    if (!is_null($stateId)) {
      // code...
      $ci->db->where('state_id', $stateId);
    }
    $cityList = $ci->db->get()->result_array();
...
/**
 * @author Leo: Get Commission fee
 * @return float 
*/
if (!function_exists('getCommissionFee')) {
  
  function getCommissionFee($userId = null) {
    $ci =& get_instance();
    if (is_null($userId)) {
      $userId = $ci->session->userdata('id');
    }
    // $user_info = $ci->db->where('id', $userId)->get('users')->row_array();                      
    // $user_type = $user_info['you_are_appling_as'];
    $user_type = $ci->session->userdata('you_are_appling_as');
    $init_fee = 0.02;
    if($user_type == '9') {
        $com_fee = 0.011;
    }
    else {
        $subscription_detail = $ci->db->where('subscriber_id', $userId)->get('subscription_details')->row_array();
        $subscription = $subscription_detail['subscription_id'];
        
        $where = array('user_type =' => (int)$user_type, 'subscription =' => (int)$subscription);
        $result_fee = $ci->db->where($where)->get('commission_ta')->row_array();
...
/**
 * @author Leo: Get Transaction Fee
 * @return float 
*/
if (!function_exists('getTransactionFee')) {
  
  function getTransactionFee($user_type = null) {
    $ci =& get_instance();
    ...
    $init_fee_t = 0.01;
    $result_fee_t = $ci->db->where('user_type',$user_type)->get('transaction_fee_1')->row_array();
