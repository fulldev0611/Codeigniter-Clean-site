...
if (!function_exists('get_user_currency')) {
    ...
    function get_user_currency() {
        ...
        if ($ci->session->userdata('id')) {
            ...
            $currency = $ci->db->where('id', $user_id)->select('currency_code')->get($table)->row_array();
            $currency_code = $currency['currency_code'];
        }
        else {
            $currency_code = "USD";
        }
        $currency_rate = $ci->db->where('currency_code', $currency_code)->get('currency_rate')->row_array();
       
        $data['user_currency_code'] = $currency_code;
        $data['user_currency_rate'] = $currency_rate['rate'];
        $data['user_currency_sign'] = currency_code_sign($currency_code);
        return $data;
    }
