...
/**
 * @author Leo: Delivery Service Model
 * */

class DeliveryServiceModel extends CI_Model
{
    private $primaryKey = 'id';
    private $name = "delivery_services";
    ...
    {
        $inputs["unique_id"] = hexdec(uniqid());
        $this->db->insert($this->name, $inputs);
        return $this->db->insert_id();
    }

    public function insert_serviceimage($image)
    {
        $this->db->insert('delivery_services_image', $image);
        $this->db->where(array(
    ...
    {
        $this->db->select("s.id,s.user_id,s.service_title,s.service_amount,s.mobile_image,c.category_name,s.rating_count,s.service_location");
        $this->db->from($this->name.' s');
    ...
    /** 
     * @author Leo: get Service Details
    */
    public function get_service_details($id)
    {
        $this->db->select()->from($this->name." s");
        $this->db->where(['status'=>1,'s.id'=>$id]);
        ...
        $this->db->reset_query();
        $this->db->select("s.*,IFNULL(si.service_image,s.service_image) as service_image,IFNULL(si.service_details_image,s.service_details_image) as service_details_image,IFNULL(si.thumb_image,s.thumb_image) as thumb_image,IFNULL(si.mobile_image,s.mobile_image) as mobile_image,c.category_name,c.category_image,r.rating");
    ...
    /**
     * @author Leo: service list 
     * @param condition array
     * @return service list array
    */
    public function getServiceList($params = array()) {
        ...
        if (isset($params["md5_ids"])) {
            $this->db->where_in("md5(s.id)", $params['md5_ids']);
        }
    ...
    {
        return $this->db->where('id', $inputs)->get($this->name)->row_array();
    }

    public function get_category()
    {
        return $this->db->get('delivery_categories')->result_array();
    }

    public function service_image($service_id)
    {
        $this->db->select();
        $this->db->from('delivery_services_image');
    ...
    public function update_service($inputs, $where)
    {
        $this->db->set($inputs);
        $this->db->where($where);
        $this->db->update('delivery_services');
        return $this->db->affected_rows() != 0 ? true : false;
    }
    ...
    public function update_service_image($inputs, $where)
    {
        $this->db->set($inputs);
        $this->db->where($where);
        $this->db->update('delivery_services_image');
        return $this->db->affected_rows() != 0 ? true : false;
    }
    ...
    {
        $this->db->select("s.id,s.user_id,s.service_title,s.service_amount,s.mobile_image,c.category_name,c.category_image,r.rating");
    ...
    {
        $this->db->select("s.id,s.user_id,s.service_title,s.service_amount,s.mobile_image,s.currency,c.category_name,c.category_image,r.rating");
        $this->db->from('delivery_services s');
        $this->db->join('categories c', 'c.id = s.category', 'LEFT');
        $this->db->join('rating_review r', 'r.service_id = s.id', 'LEFT');
        $this->db->where("s.status = 1");
    ...
    /**
     * @author Leo: get popular service list
     * @created: 
    */
    public function getPopularServices($params = array()) {
        ...
        if (isset($params['except_id']) && !empty($params['except_id'])) {
            $this->db->where('s.id!=', $params['except_id']);
        }
        if (isset($params['category']) && !empty($params['category'])) {
            $this->db->where('s.category=', $params['category']);
        }
        if (isset($params['subcategory']) && !empty($params['subcategory'])) {
            $this->db->where('s.subcategory=', $params['subcategory']);
        }
    ...
    {
        return $this->db->where("status", 1)->get('delivery_services')->result_array();
    }
    ...
    {
        $value = $this->db->select('tab_1.id,tab_1.rating,tab_1.review,tab_1.created')->select('tab_2.name as user_name,tab_2.profile_img as user_image,tab_3.name as provider_name,tab_3.profile_img as provider_image,tab_4.service_title,tab_5.name as type_name')->from('rating_review tab_1')->join("users tab_2", "tab_1.user_id=tab_2.id", "LEFT")->join("providers tab_3", "tab_1.provider_id=tab_3.id", "LEFT")->join("delivery_services tab_4", "tab_1.service_id=tab_4.id", "LEFT")->join("rating_type tab_5", "tab_1.type=tab_5.id", "LEFT")->where('tab_1.status', 1)->order_by('tab_1.id', 'DESC')->get()->result_array();
        return $value;
    }
    ...
    public function get_review_filter($service, $provider, $user, $type, $from, $to)
    {
        ...
        $this->db->join("delivery_services tab_4", "tab_1.service_id=tab_4.id", "LEFT");
    ...
    {
        return $this->db->get_where('rating_type', array(
            'id' => $id
        ))->row_array();
    }
    ...
    public function provider_list()
    {
        ...
        if ($_POST['length'] != - 1) $this->db->where('delete_status', 0);
    ...
    public function services_count($user_id)
    {
        ...
        return $this->db->count_all_results('delivery_services');

    }

    public function provider_limit($user_id)
    {
        $this->db->from('providers');
        $this->db->where('id', $user_id);
        $res = $this->db->get()->row();
        return $res->service_limit;
    }

    public function change_limit($id, $type)
    {
        $this->db->set('service_limit', 'service_limit + ' . $type, false);
        $this->db->where('id', $id);
        return $this->db->update('providers');
    }
    ...
    /*service filter*/
    public function service_filter($service_title, $category, $subcategory, $from, $to, $status = 1)
    {
        if (!empty($from))
        {
            $from_date = date("Y-m-d 00:00:00", strtotime($from));
    ...
    public function service_list($status = 1)
    {
        $this->db->select('S.*,C.category_name');
        $this->db->from('delivery_services S');
        $this->db->join('categories C', 'C.id=S.category', 'left');
    ...
    public function service_list_all()
    {
        ...
        $this->db->from('delivery_services');
        $this->db->where('status', 1);
        return $this->db->count_all_results();
    }
    ...
    public function search_all($service_title, $category)
    {
        $this->db->select('S.*,C.category_name');
        $this->db->from('delivery_services S');
        $this->db->join('categories C', 'C.id=S.category', 'left');
    ...
    {
        $ret = $this->db->select('*')->from('book_service')->where('service_id', $id)->where_not_in('status', [5, 7, 6])->get()->result();
    ...
    public function delete_row($table, $conn)
    {
        $this->db->where($conn);
    ...
    /**
     * @author Leo: delete delivery_services and images by category
    */
    public function deleteByCategory($category_id) {
        ...
        $service_ids = array();
        foreach($rows as $row) {
            @unlink(realpath($row['service_image']));
            @unlink(realpath($row['service_details_image']));
            @unlink(realpath($row['mobile_image']));
            @unlink(realpath($row['thumb_image']));
            array_push($service_ids, $row['id']);
        }

        // delete service images 
        if($this->db->where('category', $category_id)->delete($this->name)) {
            $this->db->reset_query(); 
            $this->db->select()->from("delivery_services_image")->where_in('service_id', $service_ids);
            ...
            return $this->db->where_in('service_id', $service_ids)->delete("delivery_services_image");
        }
