...
class Dashboard_model extends CI_Model
{
    ...
    {
        $query = $this->db->query(
            "select max(created) from (select max(created) as created from request_details union select max(created) as created from provider_details) AS created"
        );
    ...
    var $order = ["U.id" => "DESC"]; // default order
    var $users = "users U";
    var $subscription_details = "subscription_details SD";
    var $subscription = "subscription_fee S";
    var $admincolumn_order = [null, "U.username", "U.full_name"];
    var $admincolumn_search = ["U.username", "U.full_name"];
    var $adminorder = ["U.user_id" => "DESC"]; // default order
    var $administrators = "administrators U";

    private function p_get_datatables_query()
    {
        $this->db->select(
            "U.id,U.name,U.l_name,U.profile_img,U.mobileno,U.email,U.you_are_appling_as,U.token,created_at as created_at,U.last_login,U.status,U.type,S.subscription_name,SD.subscriber_id"
        );
        $this->db->from($this->users);
        $this->db->join(
            $this->subscription_details,
            "SD.subscriber_id=U.id",
            "left"
        );
            ...
            if ($_POST["search"]["value"]) {
                ...
                if ($i === 0) {
                    ...
                    if ($item == "status") {
                        ...
                        if (
                            strtolower($_POST["search"]["value"]) == "inactive"
                        ) {
                            $search_val = 0;
                            $this->db->or_like($item, $search_val);
                        }
    ...
    public function user_filter($username=null, $email=null, $from=null, $to=null)
    {
        if (!empty($from)) {
        ...
        $this->db->select("U.*,A.address,C.country_name,ST.name as state_name,CI.name as city_name,S.subscription_name,S.fee_description as subscription_description,SD.subscriber_id,SD.subscription_date,SD.expiry_date_time as subscription_expiry_date,UP.name as permission");
        $this->db->from("users U");
        $this->db->join("user_address A", "A.user_id=U.id", "LEFT");
        $this->db->join("country_table C", "C.id=A.country_id", "LEFT");
        ...
        $this->db->join("user_permission UP", "UP.id=U.permission", "LEFT");
        if (!empty($username)) {
    ...
    public function users_name($username, $email)
    {
        ...
        $this->db->from("users U");
        $this->db->join(
            "subscription_details SD",
            "SD.subscriber_id=U.id",
            "left"
        );
        $this->db->join(
            "subscription_fee S",
            "S.id=SD.subscription_id",
            "left"
        );
        return $this->db
    ...
    public function admin_details($user_id)
    {
        $results = [];
        $results = $this->db
            ->get_where("users", ["user_id" => $user_id])
            ->row_array();
        return $results;
    }
    ...
    public function update_profile($data)
    {
        ...
        $results = $this->db->update("users", $data, ["user_id" => $user_id]);
        return $results;
    }
    ...
    {
        $this->db->select(
            "b.*,b.currency_code as currency_code1,s.service_title,s.service_image,s.service_amount,s.rating,s.service_image,c.category_name,sc.subcategory_name,p.profile_img,p.mobileno,p.name"
        );
    ...
    {
        $ret = $this->db
            ->select("sum(fee) as paid_amt")
            ->from("subscription_details_history as s")
            ->join("subscription_fee as f", "f.id=s.subscription_id")
            ->get()
            ->row()->paid_amt;
        return $ret;
    }
    ...
    private function p_get_datatables_adminquery()
    {
        ...
        foreach (
            ...
            if ($_POST["search"]["value"]) {
                ...
                if (count($this->admincolumn_search) - 1 == $i) {
                    //last loop
                    $this->db->group_end();
                } //close bracket
            }
        ...
        if (isset($_POST["order"])) {
            // here order processing
            $this->db->order_by(
                $this->admincolumn_order[$_POST["order"]["0"]["column"]],
                $_POST["order"]["0"]["dir"]
            );
        } elseif (isset($this->adminorder)) {
