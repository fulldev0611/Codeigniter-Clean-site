...
class Admin_model extends CI_Model
{
  ...
  public function update_data($table, $data, $where = []) {
   ...
   public function getSingleData($table,$where=array()) {
	...
	/**
	 * @author Leo: delivery categories list
	 * */
	public function delivery_categories_list()
	{
		$query = $this->db->query(" SELECT * FROM `delivery_categories` WHERE `status` = 1 ")->result_array();
		return $query;
	}

	/**
	 * @author Leo: delivery categories list filter
	 * */
	public function delivery_categories_list_filter($category,$from_date,$to_date){
        if(!empty($from_date)) {
			$from_date=date("Y-m-d", strtotime($from_date));
		}else{
			$from_date='';
	...
	/**
	 * @author Leo: delivery categories details
	 * */
	public function delivery_categories_details($id)
	{
		return $this->db->get_where('delivery_categories',array('id'=>$id))->row_array();
	}
