...
class Api_model extends CI_Model {
    ...
    public function subscription_success($inputs) {
        ...
        $new_details = array();
        $user_info = $this->get_token_info($inputs['token']);
        ...
        if (!empty($record)) {
            ...
            if ($record['duration'] == 0 ) {
                $expiry_date_time = date('Y-m-d H:i:s', strtotime(date("Y-m-d  H:i:s", strtotime($subscription_date)) . " + 1hours"));
            }
            else {
            ...
            $new_details['type'] = $user_info->type;
...
/ 2), 2) )) AS distance");
        ...
        if (!empty($booking_id)) {
            ...
            if (!empty($booking)) {
                ...
                if($wallet['id']=='')
                {
                    ...
                    $insert_main_wallet['currency_code'] = $user_info['currency_code'];
                    $insert_main_wallet['user_provider_id'] = $user_info['id'];
                    $insert_main_wallet['type'] = $user_info['type'];
                    $insert_main_wallet['wallet_amt'] = 0;
                    $wallet_result = $this->db->insert('wallet_table', $insert_main_wallet);
                    $wallet['wallet_amt'] = 0;
                    $wallet['currency_code'] = $insert_main_wallet['currency_code'];
                }
    ...
    //added by MaksimU for get fees ===============================================================
    public function get_sale_fee($booking_id) {
        $sql  = " SELECT sv.user_id AS saler_id, saler.you_are_appling_as AS saler_applying_as ";
        $sql .= " FROM book_service bs ";
        $sql .= " INNER JOIN services sv ON bs.service_id = sv.id ";
        $sql .= " INNER JOIN users saler ON sv.user_id = saler.id ";
        $sql .= " WHERE bs.id = ".$booking_id;
        $query = $this->db->query($sql);
        $saler = $query->row();

        $fee_type = SALES_FEE;
        if(!empty($saler)){
            $saler_applying_as = $saler->saler_applying_as;
            if($saler_applying_as == C_YOUARE_ORGANIZATION){ // when saler is organization
                $fee_type = COMMISSION;
            }
        }

        $query = $this->db->query('select * from fees where type='.$fee_type);
        $sales_fee_ary = $query->row();

        $pertage = 0;
        if(!empty($sales_fee_ary)){
            $pertage = $sales_fee_ary->fee;
        }
    ...
    public function user_accept_history_flow($booking_id) {
        ...
        if (empty($booking) || is_null($booking_id)) {
            return;
        }

        $provider = $this->get_user_info($booking['provider_id'], 1);
        
        // added by MaksimU for employee, organization, partner ===================================================
        if(empty($provider)){
            $provider = $this->db->select('*')->from('users')->where('id', $booking['provider_id'])->get()->row_array();
        }
        // end by MaksimU =========================================================================================
        ...
        {
            $insert_main_wallet = array();
            $insert_main_wallet['token'] = $provider['token'];
            $insert_main_wallet['currency_code'] = $provider['currency_code'];
            $insert_main_wallet['user_provider_id'] = $provider['id'];
            $insert_main_wallet['type'] = $provider['type'];
            $insert_main_wallet['wallet_amt'] = 0;
            $wallet_result = $this->db->insert('wallet_table', $insert_main_wallet);
            $wallet['wallet_amt'] = 0;
            $wallet['currency_code'] = $insert_main_wallet['currency_code'];
        ...
        $current_wallet = get_gigs_currency($wallet['wallet_amt'], $wallet['currency_code'], $booking['currency_code']);

        // changed by MaksimU for sales fee or commission ==============================
        ...
        $pertage = $this->get_sale_fee($booking_id);
        $sales_fee = ($booking['amount']) * $pertage / 100;
        $ComAmount = $booking['amount'] - $sales_fee;
        ...
        // changed by MaksimU for transaction fee ======================================================

        // $pertage = 2;
        // $commission = ($booking['amount']) * $pertage / 100;
        $fee_type = TRANSACTION_FEE;
        $query = $this->db->query('select * from fees where type='.$fee_type);
        $transaction_fee_ary = $query->row();
        $pertage = 0;
        if(!empty($transaction_fee_ary)){
            $pertage = $transaction_fee_ary->fee;
        }
        $transaction_fee = ($booking['amount']) * $pertage / 100;
        ...
        $history_pay['card_id'] = 'Self';
        $history_pay['total_amt'] = $booking['amount'] * 100;
        $history_pay['fee_amt'] = $transaction_fee;                         //changed by MaksimU
        $history_pay['net_amt'] = $booking['amount'] * 100;
        $history_pay['amount_refund'] = 0;
        $history_pay['current_wallet'] = $current_wallet;
        $history_pay['credit_wallet'] = 0;
        $history_pay['debit_wallet'] = $transaction_fee;                    //changed by MaksimU
        $history_pay['avail_wallet'] = $current_wallet - $transaction_fee;  //changed by MaksimU
        $history_pay['reason'] = COMPLETE_PROVIDER;
        $history_pay['created_at'] = date('Y-m-d H:i:s');
        ...
        if ($this->db->insert('wallet_transaction_history', $history_pay)) {
            ...
            $commissionInsert = [
                ...
                'currency_code' => $booking['currency_code'],
                'amount' => $booking['amount'],
                'commission' => $pertage,  //changed by MaksimU
            ];
            $commInsert = $this->db->insert('revenue', $commissionInsert);
        }
    }
    ...
    // Product Insert Fee 0.2  by Vadim 
    public function product_insert_wallet_history_flow($totalPrice,$user_currency_code ,$token) {
                ...
                $curren_wallet = get_gigs_currency($wallet['wallet_amt'], $wallet['currency_code'], $user_currency_code);

                   /* wallet infos */

                $history_pay['token'] = $token;
				$history_pay['currency_code']=$user_currency_code;
                $history_pay['user_provider_id'] = $user_info->id;
                $history_pay['type'] = $user_info->type;
                $history_pay['tokenid'] = $token;
                $history_pay['payment_detail'] = "Insertion Product pay"; //response
                $history_pay['charge_id'] = "1";
                $history_pay['transaction_id'] = "10";
                $history_pay['exchange_rate'] = 0;
                $history_pay['paid_status'] = 'pass';
                $history_pay['cust_id'] = 'Self';
                $history_pay['card_id'] = 'Self';
                $history_pay['total_amt'] = 1;
                $history_pay['fee_amt'] = 0;
                $history_pay['net_amt'] = 1;
                $history_pay['amount_refund'] = 0;
                $history_pay['current_wallet'] = $curren_wallet;
                $history_pay['credit_wallet'] = 0;
                $history_pay['debit_wallet'] = ($totalPrice);
                $history_pay['avail_wallet'] = $curren_wallet - ($totalPrice);
                $history_pay['reason'] = "Insertion Product Fee";
                $history_pay['created_at'] = date('Y-m-d H:i:s');
    ...
    // Priority Service Fee 12 pount per Month  by Vadim 
    public function priority_wallet_history_flow ($totalPrice,$user_currency_code ,$token) {    
        ...
        $history_pay['type'] = $user_info->type;
        $history_pay['tokenid'] = $token;
        $history_pay['payment_detail'] = "priority service fee"; //response
        $history_pay['charge_id'] = "1";
        $history_pay['transaction_id'] = "11";
        $history_pay['exchange_rate'] = 0;
        $history_pay['paid_status'] = 'pass';
        $history_pay['cust_id'] = 'Self';
        $history_pay['card_id'] = 'Self';
        ...
        $history_pay['debit_wallet'] = ($totalPrice);
        $history_pay['avail_wallet'] = $curren_wallet - ($totalPrice);
        $history_pay['reason'] = "Priority Service Fee 12/M";
        $history_pay['created_at'] = date('Y-m-d H:i:s');
