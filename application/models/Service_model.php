...
class Service_model extends CI_Model
{
    ...
    */
    ...
    /**
     * @author Leo: delete services and images by category
    */
    public function deleteByCategory($category_id) {
        ...
        if($this->db->where('category', $category_id)->delete($this->name)) {
            $this->db->reset_query(); 
            $this->db->select()->from("services_image")->where_in('service_id', $service_ids);
            ...
            return $this->db->where_in('service_id', $service_ids)->delete("services_image");
        }
    ...
    /** 
     * @author Vadim: get Category Details
    */
   ...
   /** 
     * @author Vadim: get Service list where subcategroies 
    */
    public function get_service_sub($subcategory)
    {
        $this->db->select("s.id,s.user_id,s.service_title,s.service_amount,s.mobile_image,c.category_name,sc.subcategory_name,s.rating_count,s.service_location,s.serviceamounttype");
        $this->db->from('services s');
        $this->db->join('categories c', 'c.id = s.category', 'LEFT');
        $this->db->join('subcategories sc', 'sc.id = s.subcategory', 'LEFT');
        $this->db->where("s.status = 1");
