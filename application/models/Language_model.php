...
class Language_model extends CI_Model
{
    ...
    var $column_order = [null, "L.lang_key", "L.lang_value", "L.language"];
    var $column_search = ["L.lang_key", "L.lang_value", "L.language"];
    var $order = ["L.sno" => "DESC"]; // default order
    ...
    public function language_list($page_key)
    {
        ...
        $this->db->group_by(["L.page_key", "L.lang_key"]);
        $query = $this->db->get();
    ...
    public function currenct_page_key_value($inputs)
    {
        ...
        if (!empty($my_keys)) {
            $this->db->select(
                "sno,lang_key,lang_value,language,placeholder,validation1,validation2,validation3,type,page_key"
            );
            ...
            if (!empty($my_final)) {
                foreach ($my_final as $keyvalue) {
                    ...
                    $my_final_values[$keyvalue["lang_key"]][
                        $keyvalue["language"]
                    ]["name"] = $keyvalue["lang_value"];
                    $my_final_values[$keyvalue["lang_key"]][
                        $keyvalue["language"]
                    ]["placeholder"] = $keyvalue["placeholder"];
                    $my_final_values[$keyvalue["lang_key"]][
                        $keyvalue["language"]
                    ]["validation1"] = $keyvalue["validation1"];
                    $my_final_values[$keyvalue["lang_key"]][
                        $keyvalue["language"]
                    ]["validation2"] = $keyvalue["validation2"];
                    $my_final_values[$keyvalue["lang_key"]][
                        $keyvalue["language"]
                    ]["validation3"] = $keyvalue["validation3"];
                    $my_final_values[$keyvalue["lang_key"]][
                        $keyvalue["language"]
                    ]["type"] = $keyvalue["type"];
                    $my_final_values[$keyvalue["lang_key"]][
                        $keyvalue["language"]
                    ]["lang_key"] = $keyvalue["lang_key"];
                }
            }
        }
    ...
    {
        $query = $this->db->query(
            "SELECT language, language_value,tag FROM `language` WHERE status = 1"
        );
        $result = $query->result_array();
        return $result;
    }
    ...
    public function keyword_model()
    {
        if (!empty($_POST["field_name"])) {
            ...
            if ($record >= 1) {
                ...
                $datas["placeholder"] = !empty($_POST["placeholder"])
                    ? $_POST["placeholder"]
                    : "";
                $datas["validation1"] = !empty($_POST["validation1"])
                    ? $_POST["validation1"]
                    : "";
                $datas["validation2"] = !empty($_POST["validation2"])
                    ? $_POST["validation2"]
                    : "";
                $datas["validation3"] = !empty($_POST["validation3"])
                    ? $_POST["validation3"]
                    : "";
                $datas["language"] = "en";
