...
class User_login_model extends CI_Model
{
    ...
    public function isset_mobile_otp($mobile_no, $country_code, $otp_data)
    {
        ...
        if ($ret > 0) {
            ...
            $save_otp = $this->db->update("mobile_otp", [
                "endtime" => $otp_data["endtime"],
                "otp" => $otp_data["otp"],
                "updated_on" => utc_date_conversion(date("Y-m-d H:i:s")),
            ]);
        } else {
    ...
    public function is_valid_login($username, $password)
    {
        ...
        $this->db->select(
            "username,user_id, latitude, longitude, profile_img, register_through"
        );
        $this->db->from("users");
        $this->db->where(
            "(username = '$username' OR email = '$username' OR mobile_no = '$username')"
        );
    ...
    public function signup($inputs)
    {
        ...
        $this->db->update("users", ["unique_code" => $unique_code]);
        return $result;
    }
    ...
    public function temp_subscription_success($inputs)
    {
        ...
        $record = $this->db
            ->get_where("subscription_fee", ["id" => $subscription_id])
            ->row_array();
        ...
        if (!empty($record)) {
            ...
            $expiry_date_time = date(
                "Y-m-d H:i:s",
                strtotime(
                    date("Y-m-d  H:i:s", strtotime($subscription_date)) .
                        " +" .
                        $days .
                        "days"
                )
    ...
    public function forgot_password($email)
    {
        ...
        if (!empty($record)) {
            ...
            $this->db->update("users", ["forgot" => $userid]);

            $this->load->library("servpro");
            $to = $email;
            $subject = "Forgot Password reset link for ";
            $message =
                'Reset Password link is follows <a href="' .
                base_url() .
    ...
    public function otp_check_uemail($inputs = "")
    {
        ...
        # added by maksimU for staff editing
    ...
    {
        $this->db
            ->where(["country_code" => $inputs["countryCode"], "status" => 1])
            ->like("mobileno", $inputs["mobileno"], "match", "after");
        return $this->db->count_all_results("providers");
    }
    ...
    public function otp_validation($user_data, $input_data)
    {
        ...
        if ($time_count == 1) {
            $this->db->where($user_data);
            $this->db->update("mobile_otp", ["status" => 0]);
            ...
            if ($result) {
                ...
                $this->db->update("providers", ["token" => $token]);
                $profile_img = base_url() . "assets/img/professional.png";

                $this->db->select(
                    'id,name,email,mobileno,category,subcategory,IF(profile_img IS NULL or profile_img = "", "' .
                        $profile_img .
                        '", profile_img) as profile_img,token'
                );
                $this->db->where("id", $user_id);
                $records = $this->db->get("providers")->row_array();
            }
            return ["data" => $records, "msg" => "ok"];
        } else {
            $this->db->where($user_data);
            $this->db->update("mobile_otp", ["status" => 0]);
            return "otp_expired";
    ...
    public function insertemailusers($input_data)
    {
        if ($input_data) {
            ...
            $this->session->set_userdata("user_last_insert_id", $last_query);
            $result2 = $this->db->insert("providers", $input_data);
            $last_query2 = $this->db->insert_id();
            $records = [];
            if ($result) {
                $user_id = $last_query;
                $user_id2 = $last_query2;
                $token = $this->getToken(14, $user_id);
                ...
                $this->db->select(
                    'id,name,email,mobileno,IF(profile_img IS NULL or profile_img = "", "' .
                        $profile_img .
                        '", profile_img) as profile_img,token'
                );
                $this->db->where("id", $user_id);
    ...
    public function get_user_detailsbymail($email)
    {
        $record = $this->db
            ->where("email", $email)
            ->get("users")
