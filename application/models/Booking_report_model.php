...
class Booking_report_model extends CI_Model
{
    ...
    public function get_total_bookings()
    {
        ...
        $this->db->select('tab_1.id,tab_1.location,tab_1.service_date,tab_1.currency_code,tab_1.amount,tab_1.currency_code,tab_1.notes,tab_1.latitude,tab_1.longitude,tab_1.request_date,tab_1.request_time,tab_1.status,tab_1.reason,tab_1.updated_on');
    ...
    public function get_reject_bookings()
    {
        ...
        $this->db->select('tab_1.id,tab_1.location,tab_1.service_date,tab_1.currency_code,tab_1.amount,tab_1.currency_code,tab_1.notes,tab_1.latitude,tab_1.longitude,tab_1.request_date,tab_1.request_time,tab_1.status,tab_1.reason,tab_1.updated_on,,tab_1.admin_change_status,tab_1.reject_paid_token,tab_1.admin_reject_comment');
        $this->db->select('tab_2.service_title');
        $this->db->select('tab_3.name as user_name,tab_3.mobileno as user_mobile,tab_3.email as user_email,tab_3.profile_img as user_profile_img');
        $this->db->select('tab_4.name as provider_name,tab_4.mobileno as provider_mobile,tab_4.email as provider_email,tab_4.profile_img as provider_profile_img');
        $this->db->from('book_service tab_1');
    ...
    public function get_filter_cancel_bookings($service_id, $status, $user_id, $provider_id, $from, $to)
    {
        ...
        $this->db->select('tab_1.id,tab_1.location,tab_1.service_date,tab_1.currency_code,tab_1.amount,tab_1.currency_code,tab_1.notes,tab_1.latitude,tab_1.longitude,tab_1.request_date,tab_1.request_time,tab_1.status,tab_1.reason,tab_1.updated_on,tab_1.admin_change_status');
        $this->db->select('tab_2.service_title');
        $this->db->select('tab_3.name as user_name,tab_3.mobileno as user_mobile,tab_3.email as user_email,tab_3.profile_img as user_profile_img');
        $this->db->select('tab_4.name as provider_name,tab_4.mobileno as provider_mobile,tab_4.email as provider_email,tab_4.profile_img as provider_profile_img');
        $this->db->from('book_service tab_1');
    ...
    public function get_reject_bookings_by_id($id)
    {
        ...
        $this->db->select('tab_1.id,tab_1.location,tab_1.service_date,tab_1.currency_code,tab_1.amount,tab_1.currency_code,tab_1.notes,tab_1.latitude,tab_1.longitude,tab_1.request_date,tab_1.request_time,tab_1.status,tab_1.reason,tab_1.updated_on,tab_1.status as payment_status');
        $this->db->select('tab_2.service_title');
    ...
    public function get_sale_fee($booking_id)
    {
        ...
        if (!empty($saler))
        {
            ...
            if ($saler_applying_as == C_YOUARE_ORGANIZATION)
            { // when saler is organization
                $fee_type = COMMISSION;
            }
        }
        ...
        if (!empty($data['book_id']))
        {
            ...
            if (!empty($booking))
            {
                ...
                if ($user_info->type == 1)
                {
                    ...
                    $pertage = $this->get_sale_fee($data['book_id']);
                    $cash_text = "Rejected by Provider and refunded by Admin.";
                ...
                if ($this->db->insert('wallet_transaction_history', $history_pay))
                {
                    if ($user_info->type == 1)
                    {
                        ...
                        $commissionInsert = ['date' => date('Y:m:d') , 'provider' => $booking['provider_id'], 'user' => $booking['user_id'], 'currency_code' => $booking['currency_code'], 'amount' => $booking['amount'], 'commission' => $pertage, ];
    ...
    public function get_book_info($book_service_id)
    {
        ...
        $ret = $this->db->select('tab_1.provider_id,tab_1.user_id,tab_1.status,tab_1.currency_code,tab_1.amount,tab_2.service_title')->from('book_service as tab_1')->join('services as tab_2', 'tab_2.id=tab_1.service_id', 'LEFT')->where('tab_1.id', $book_service_id)->limit(1)->order_by('tab_1.id', 'DESC')->get()->row_array();
        return $ret;
    ...
    public function get_wallet($token)
    {
        ...
        $wallet_tot = $this->db->select('sum(credit_wallet)as total_credit,sum(debit_wallet)as total_debit')->from('wallet_transaction_history')->where('token', $token)->order_by('id', 'DESC')->get()->row_array();
