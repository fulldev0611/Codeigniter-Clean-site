...
class Employee_login extends MY_Controller {
   ...
   public function __construct() {
	...
	# view login page for employee
	public function index()
	{ 
		//die("====");
	    if (empty($this->session->userdata['id']) || empty($this->session->userdata('data_employee')))
	    {
	...
	public function employee_login_check()
	{ 
		...
		$provider_details = $this->user_login->get_provider_detailsbymail($email);
         ...
         if($num_rows == 1)
        {
        	...
        	redirect(base_url()."employee?msg=wrong");
        }   
