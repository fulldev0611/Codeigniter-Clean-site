...
class Dashboard extends MY_Controller
{
    ...
    */
    public function create_paypal_user_wallet()
    {
        ...
        $request->body = array(
            ...
            'cancel_url' => base_url() . 'employee/dashboard/paypal_add_wallet'],
    ...
    */
    ...
    {
        $this->data['page'] = 'employee_settings';
        $this->data['details'] = $this->employee->getDetail($this->session->userdata('id'));
        ...
        $this->data['selected_skill'] = $user_meta[0]['skill_id'];
        $skill_list = $this->user_skill->getSkillList();     
    ...
    /**
     * @author Leo: Security Setting
    */
    public function employee_security()
    {
        $this->data['page'] = 'employee_security';
    ...
    public function employee_jobs()
    {
        ...
        $cat_id = $row1->service_values;

        $conditions['where']['b.category_id'] = $cat_id;

        $conditions['returnType'] = 'count';
        $totalRec = $this->booking->getRows1($conditions);
        ...
        $this->data['all_bookings'] = $this->booking->getRows1($conditions);
    ...
    function prf_crop($av_src, $av_data, $av_file, $req_height, $req_width, $table_name, $redirect)
    {
        if ($table_name == "employee") {
            $directoryName = 'uploads/employee_img/';
        } else {
        ...
        $data['cropped_fliepath'] = $directoryName . $cropfliename;

        $id = $this->session->userdata('id');
        if ($table_name == "employee") {
            $employDate['recognition_img'] = $directoryName . $cropfliename;
            $this->employee->updateEmployee($id, $employDate);
        } else {
            $table_data['profile_img'] = $directoryName . $cropfliename;
            $this->db->where('id', $id);
    ...
    public function update_user()
    {
        if (!empty($_POST))
        {
            ...
            if ($this->db->update('users', $table_data))
            {
                ...
                $employeeData['work_time'] = $_POST['work_time'];

                if ($_POST['work_time']=="Part-time" && !empty($_POST['work_day'])) {
                    $employeeData['work_day'] = json_encode($_POST['work_day']);
                }

                if ($_POST['work_time']=="Part-time" && !empty($_POST['work_start'])) {
                    $employeeData['work_start'] = $_POST['work_start'];
                }

                if ($_POST['work_time']=="Part-time" && !empty($_POST['work_end'])) {
                    $employeeData['work_end'] = $_POST['work_end'];
                }

                $this->employee->updateEmployee($id, $employeeData);

                if (!empty($_POST['skill_id'])) {
                    $skill_data = array();
                    $skill_data['user_id'] = $id;
                    $skill_data['skill_id'] = $_POST['skill_id'];
    ...
    public function employee_wallet()
    {
        $this->data['page'] = 'employee_wallet';
        $this->data['wallet'] = $this->api->get_wallet($this->session->userdata('chat_token'));
        $this->data['wallet_history'] = $this->api->get_wallet_history_info($this->session->userdata('chat_token'));
        $this->load->vars($this->data);
        $this->load->view($this->data['theme'] . '/template');
    ...
    /* Leo: employee, sole-trader, self-employed subscription */
    public function subscription()
    {
        if (empty($this->session->userdata('id'))) {
            redirect(base_url());
        }
        $roleArr = [C_YOUARE_SOLETRADER, C_YOUARE_EMPLOYEE, C_YOUARE_SELF_EMPLOYED];
        if(!in_array($this->session->userdata('you_are_appling_as'), $roleArr)){
            redirect(base_url());
        }
        $user_id = $this->session->userdata('id');
     ...
     * @created: 
    */
    public function create_stripe_soletrader_subscription()
    {
        ...
        $session = \Stripe\Checkout\Session::create(['customer_email' => $this->session->userdata['email'], 'payment_method_types' => ['card'], 'line_items' => [['price_data' => ['currency' => $data['currency_code'], 'product_data' => ['name' => $data['subscription_name'], ], 'unit_amount' => 100, ], 'quantity' => $quantity, ]], 'mode' => 'payment', 'success_url' => base_url() . 'soletrader-subscription-success/{CHECKOUT_SESSION_ID}?sub_id=' . $subscriptionId, 'cancel_url' => base_url() . 'soletrader-subscription', ]);
     ...
     * @created: 
    */
    public function soletrader_subscription_success($sessionId)
    {
        // valid check
        $row = $this->db->select()->where('tokenid', $sessionId)->get('subscription_payment')->row_array();
        if (is_array($row) && count($row) > 0)
        {
            redirect(base_url() . 'soletrader-subscription');
            return;
        }
     ...
     * @created: 
    */
    public function create_stripe_employee_subscription()
    {
        ...
        $session = \Stripe\Checkout\Session::create(['customer_email' => $this->session->userdata['email'], 'payment_method_types' => ['card'], 'line_items' => [['price_data' => ['currency' => $data['currency_code'], 'product_data' => ['name' => $data['subscription_name'], ], 'unit_amount' => 100, ], 'quantity' => $quantity, ]], 'mode' => 'payment', 'success_url' => base_url() . 'employee-subscription-success/{CHECKOUT_SESSION_ID}?sub_id=' . $subscriptionId, 'cancel_url' => base_url() . 'employee-subscription', ]);
     ...
     * @created: 
    */
    public function employee_subscription_success($sessionId)
    {
        // valid check
        $row = $this->db->select()->where('tokenid', $sessionId)->get('subscription_payment')->row_array();
        if (is_array($row) && count($row) > 0)
        {
            redirect(base_url() . 'employee-subscription');
            return;
        }
     ...
     * @created: 
    */
    public function create_stripe_self_employed_subscription()
    {
        ...
        $session = \Stripe\Checkout\Session::create(['customer_email' => $this->session->userdata['email'], 'payment_method_types' => ['card'], 'line_items' => [['price_data' => ['currency' => $data['currency_code'], 'product_data' => ['name' => $data['subscription_name'], ], 'unit_amount' => 100, ], 'quantity' => $quantity, ]], 'mode' => 'payment', 'success_url' => base_url() . 'self-employedsubscription-success/{CHECKOUT_SESSION_ID}?sub_id=' . $subscriptionId, 'cancel_url' => base_url() . 'self-employed-subscription', ]);
     ...
     * @created: 
    */
    public function self_employed_subscription_success($sessionId)
    {
        // valid check
        $row = $this->db->select()->where('tokenid', $sessionId)->get('subscription_payment')->row_array();
        if (is_array($row) && count($row) > 0)
        {
            redirect(base_url() . 'self-employed-subscription');
            return;
        }
     ...
     * @created: 
    */
    public function paypal_soletrader_subscription($subscription_id, $plan_id)
    {
        $paypalKeys = paypalKeys();
        $this->load->model('SubscriptionFee');
        $this->data['subscription'] = $this->SubscriptionFee->getData($subscription_id);
        $this->data['page'] = 'paypal_soletrader_subscription';
        $this->data['client_id'] = $paypalKeys["client_id"];
        $this->data['subscription_id'] = $subscription_id;
     ...
     * @created: 
    */
    public function paypal_employee_subscription($subscription_id, $plan_id)
    {
        $paypalKeys = paypalKeys();
        $this->load->model('SubscriptionFee');
        $this->data['subscription'] = $this->SubscriptionFee->getData($subscription_id);
        $this->data['page'] = 'paypal_employee_subscription';
        $this->data['client_id'] = $paypalKeys["client_id"];
        $this->data['subscription_id'] = $subscription_id;
        $this->data['plan_id'] = $plan_id;
        $this->load->vars($this->data);
     ...
     * @created: 
    */
    public function paypal_self_employed_subscription($subscription_id, $plan_id)
    {
        $paypalKeys = paypalKeys();
        $this->load->model('SubscriptionFee');
        $this->data['subscription'] = $this->SubscriptionFee->getData($subscription_id);
        $this->data['page'] = 'paypal_self_employed_subscription';
        $this->data['client_id'] = $paypalKeys["client_id"];
        $this->data['subscription_id'] = $subscription_id;
        $this->data['plan_id'] = $plan_id;
        $this->load->vars($this->data);
