...
class Api extends REST_Controller
{
            ...
            if (!empty($user_data['name']) || !empty($user_data['email']) || !empty($user_data['mobileno']) || !empty($user_data['country_code']) || !empty($_FILES['profile_img']))
            {
                ...
                if (!empty($user_data['type']))
                {
                    ...
                    if ($result)
                    {
                        $response_code = '200';
                        $response_message = 'Profile updated successfully';
                        $data = $this->api->user_profile(array(
                            'user_id' => $WHERE['id']
                        ));
                    }
                    else
                ...
                if ($result)
                {
                    ...
                    if (!empty($result))
                    {
                        ...
                        foreach ($result as $details)
                        {
                            ...
                            $rating_count = $this->db->where(array(
                                "service_id" => $details['service_id'],
                                'status' => 1
                            ))->count_all_results('rating_review');
    ...
    public function get_chat_list_post()
    {
        ...
        if (!empty($token))
        {
            $sent = [];
            $receive = [];
            $sent = $this->db->select('receiver_token as token')->from('chat_table')->where('sender_token', $token)->get()->result_array();
            $receive = $this->db->select('sender_token as token')->from('chat_table')->where('receiver_token', $token)->get()->result_array();
        ...
        if (!empty($token))
        {
            ...
            if ($params['paytype'] == 'stripe')
            {
                ...
                if (!empty($params['amount']) && !empty($params['tokenid']) && $params['amount'] > 0)
                {
                    ...
                    $check_card = $this->db->get_where('stripe_customer_table', array(
                        'user_token' => $token
                    ))->row();
    ...
    public function BraintreePaypal_post()
    {
        if ($this->api_token != '')
        {
            ...
            if (!empty($user_data['amount']) && !empty($user_data['payload_nonce']) && !empty($user_data['orderID']))
            {
                ...
                $gateway = new Braintree\Gateway(['environment' => 'sandbox', 'merchantId' => '', 'publicKey' => '', 'privateKey' => '']);

                if ($gateway)
                {

                    $result = $gateway->transaction()->sale(['amount' => $amount, 'paymentMethodNonce' => $payload_nonce, 'orderId' => $orderId, 'options' => ['submitForSettlement' => True], ]);
    ...
    /**
     * @author Leo: check email login user
     * 	'email'
     * 	'password'
     *
     */
    public function checkemaillogin_user_post()
    {
    ...
    /** 
     * @author Leo: send verification code to email
     *  'email'
     */
    public function otpsend_toemail_post()
    {
        $input_data = $this->input->post();
        ...
        $postData = ['promo' => $otp, 'email' => $input_data["email"], 'description' => 'verfication code'];
    ...
    /** 
     * @author Leo: after otp verfication, register user and send confirm email
     * 	'name'
     'l_name'
     'email'
     'password'
    ...
    public function register_user_post()
    {
        ...
        {
            $chk_email['email'] = $input_data['email'];
            $is_available = $this->user_login->otp_check_uemail($chk_email);
            $is_available_provider = $this->user_login->otp_check_email($chk_email);
            if ($is_available > 0 || $is_available_provider > 0)
            {
                $return = array(
                    'response' => "duplicated_email",
                    "msg" => "Duplicated Email Address"
                );
                $this->response($return, REST_Controller::HTTP_OK);
                exit;
            }
            $chk_mobile['mobileno'] = $input_data['mobileno'];
            $chk_mobile['countryCode'] = $input_data['country_code'];
            $is_available_mobile = $this->user_login->check_mobile_no($chk_mobile);
            $is_available_mobileuser = $this->user_login->check_user_mobileno($chk_mobile);
            if ($is_available_mobile > 0 || $is_available_mobileuser > 0)
            {
                $return = array(
                    'response' => "duplicated_mobileno",
            ...
            $input_data['status'] = 1;
            $input_data['share_code'] = $this->user_login->ShareCode(6, strlen($input_data['name']));
    		$input_data['currency_code'] = settings('currency');
    ...
    /**
     * @author Leo: forgot pwd mail send
     * 		email
     */
    public function send_forgot_pwd_post()
    {
        $email = $this->input->post('email');
    ...
    public function country_list_post()
    {
        ...
        $this->response($country_list, REST_Controller::HTTP_OK);
    }

    /**
     * @author Leo: Email Check.
     */
