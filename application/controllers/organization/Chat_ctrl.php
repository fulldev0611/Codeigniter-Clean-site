...
class Chat_ctrl extends CI_Controller {
  ...
  # added by maksimU : For startting Organization order
  /*
  *   This function used in Organization / View Orders
  */
  public function organization_order_new_chat()
  {     
    extract($_GET);
    ...
    $chat_room = $this->Chat_model->get_chat_room_employee_order($book_id);
  ...
  public function avcall()
  {
    $this->data['config']['VIDEOSERVER_PROTOCAL'] = $this->config->item('VIDEOSERVER_PROTOCAL');
    $this->data['config']['VIDEOSERVER_ADDRESS'] = $this->config->item('VIDEOSERVER_ADDRESS');
    $this->data['config']['VIDEOSERVER_PORT'] = $this->config->item('VIDEOSERVER_PORT');
    ...
    $this->data['users'] = [];
    // $this->data['users'][] = ['id'=>1, 
    //                           'name'=>$admin['username'] .'/'. $admin['full_name'], 
    //                           'mobileno'=>'-', 
    //                           'email'=>$admin['email'],
    //                           'user_type'=>'2'
    //                         ];
    $this->data['users'] = array_merge($this->data['users'] , $this->user_model->get_myclient_list());
    if(!empty($roomId))
    {
      ...
      //$this->call_model->update(['status'=>1, 'update_at=now()'], ['room_indentifier'=>$roomId]);
      //$this->call_model->update('status=1, update_at=now()', ['room_indentifier'=>$roomId]);
      //$this->call_model->update(['status'=>1, 'update_at'=>'now()'], 'room_identifier="'.$roomId.'"');
      $this->db->query('UPDATE `call_room` SET `status` = 1, update_at=NOW() WHERE `room_identifier` = "'.$roomId.'"');
    }
    else
    {
      $this->data['roomId'] = substr( md5(date('YndHis').$this->session->userdata('id')), 0, 10);
    }

    $this->data['theme'] = 'organization';
    $this->data['module'] = '../common/chat';
    $this->data['page'] = 'videochat';
    $this->load->vars($this->data);
    $this->load->view($this->data['theme'] . '/template');
