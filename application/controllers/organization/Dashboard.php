...
class Dashboard extends MY_Controller
{
    ...
    */
    public function create_paypal_user_wallet()
    {
        ...
        $request->body = array(
            ...
            'cancel_url' => base_url() . 'organization/dashboard/paypal_add_wallet'],
    ...
    */
    ...
    {
        $shift_data = [];
        $users = $this->db->select('name, id, profile_img')->where('you_are_appling_as', 8)->or_where('permission', 5)->get('users')->result_array();
        $this->data['page'] = 'organization_time_clock';
    ...
    {   
        $user_id = $this->session->userdata('id');
        $users = $this->db->select('name, id, profile_img', 'permission')
                    ->where('you_are_appling_as', 8)
                    ->or_where('permission', 5)
                    ->get('users')
                    ->result_array();
        $organization = $this->organization->getOrganizationByUserId($this->session->userdata('id'));
        $this->load->model('Staff_model','staff');
        ...
        $this->data['users'] = $users;
        $this->data['page'] = 'organization_job_scheduling';
        $this->load->vars($this->data);
        $this->load->view($this->data['theme'] . '/template');
    }
    public function company_work_flow()
    {
        $user_id = $this->session->userdata('id');
        $users = $this->db->select('name, id, profile_img', 'permission')
                    ->where('you_are_appling_as', 8)
        ...
        $this->data['users'] = $users;
        $this->data['page'] = 'organization_work_flow';
        $this->load->vars($this->data);
        $this->load->view($this->data['theme'] . '/template');
    }
    ...
    /**
     * @author Leo: Organization Security Setting
    */
    public function organization_security()
    {
        $this->data['page'] = 'organization_security';
     ...
     * @created: 
    */
    public function create_stripe_organization_subscription()
    {
        ...
        $session = \Stripe\Checkout\Session::create(['customer_email' => $this->session->userdata['email'], 'payment_method_types' => ['card'], 'line_items' => [['price_data' => ['currency' => $data['currency_code'], 'product_data' => ['name' => $data['subscription_name'], ], 'unit_amount' => 100, ], 'quantity' => $quantity, ]], 'mode' => 'payment', 'success_url' => base_url() . 'organization-subscription-success/{CHECKOUT_SESSION_ID}?sub_id=' . $subscriptionId, 'cancel_url' => base_url() . 'organization-subscription', ]);
     ...
     * @created: 
    */
    public function organization_subscription_success($sessionId)
    {
        // valid check
        $row = $this->db->select()->where('tokenid', $sessionId)->get('subscription_payment')->row_array();
        if (is_array($row) && count($row) > 0)
        {
            redirect(base_url() . 'organization-subscription');
            return;
        }
     ...
     * @created: 
    */
    public function paypal_organization_subscription($subscription_id, $plan_id)
    {
        $paypalKeys = paypalKeys();
        $this->load->model('SubscriptionFee');
        $this->data['subscription'] = $this->SubscriptionFee->getData($subscription_id);
        $this->data['page'] = 'paypal_organization_subscription';
        $this->data['client_id'] = $paypalKeys["client_id"];
