...
/**
 * @author: Leo 
 * @desc: Reason To Love CRUD on super admin panel
 * @created: 
*/

class Reason_to_love extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->data['model'] = 'reason_to_love';
    ...
    public function index($offset = 0)
    {
        ...
        $dataList = $this->ReasonToLoveModel->List();
    ...
    public function create()
    {
        ...
        if ($this->input->post('form_submit')) {
            if ($this->data['admin_id'] > 1) {
                ...
                redirect(base_url() . 'admin/reason_to_love');
            } else {
                $data = array();
                ...
                if (isset($_FILES) && isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])) {
                    ...
                    $upload_sts = $this->common->global_file_upload('uploads/reason_to_love_images/', 'image', time() . $filename);
                    
                    if (isset($upload_sts['success']) && $upload_sts['success'] == 'y') {
                        $uploaded_file_name = $upload_sts['data']['file_name'];
    
                        if (!empty($uploaded_file_name)) {
                            $image_url = 'uploads/reason_to_love_images/' . $uploaded_file_name;
                            // $data['image'] = image_resize(480, 320, $image_url,  $uploaded_file_name,"reason_to_love_images");
                ...
                if ($this->ReasonToLoveModel->add($data)) {
    ...
    {
        $this->data['page'] = 'edit';
        $datalist = $this->ReasonToLoveModel->get($id);
        ...
        if ($this->data['admin_id'] > 1) {
            ...
            if ($this->input->post('form_submit')) {
                ...
                if ($this->ReasonToLoveModel->update($id, $data)) {
    ...
    public function delete()
    {
        if ($this->data['admin_id'] > 1) {
            ...
            if (!empty($id)) {
                $datalist = $this->ReasonToLoveModel->get($id);
                @unlink($datalist['image']);
                $this->ReasonToLoveModel->delete($id);
