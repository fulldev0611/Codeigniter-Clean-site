...
class Dashboard extends CI_Controller
{
  ...
  public function users($value = '')
  {
    ...
    if ($this->input->get('form_submit'))
    {
      ...
      $lists = $this->dashboard->user_filter($username, $email, $from, $to);
    }
    else
    {
      $lists = $this->dashboard->user_filter();
    }
    // print_r($lists); exit;
  ...
  public function ajax_remove_user()
  {
      ...
      if(!$res)
      {
          $result = ['result'=>'failed', 'data'=>['msg'=>'could not delete.'] ];
      }
      else
      {
  ...
  public function add_user(){
      ...
      if(empty($edit_user_id))
      {
          ...
          $ret = $this->User_model->insertUser($input_data);  
      } else{
          $username = strlen($input_data['name']);
          $input_data['share_code'] = $this->user_login->ShareCode(6, $username);
          $input_data['currency_code'] = settings('currency');
          ...
          $this->User_model->updateUser($input_data);  
          $ret = 'ok';
      }
  ...
  public function user_details($value = '')
  {
    ...
    $this->data["user_details"] = $this->User_model->get_user_details($user_id);
    $this->load->vars($this->data);
    $this->load->view($this->data['theme'] . '/template');
  }
    ...
    else
    {
      ...
      foreach ($lists as $template)
      {
        ...
        $row[] = '<h2 class="table-avatar"><a href="#" class="avatar avatar-sm mr-2"> <img class="avatar-img rounded-circle" alt="" src="' . $profile_img . '"></a>
                    <a href="' . base_url() . 'user-details/' . $template->id . '">' . str_replace('-', ' ', $template->name) . (is_null($template->l_name)?"":" ".$template->l_name) . '</a></h2>';

        $row[] = $template->email;
        $row[] = (empty($template->you_are_appling_as) || !isset($applyingAsArr[intval($template->you_are_appling_as)]))?"User":$applyingAsArr[intval($template->you_are_appling_as)];
        ...
        $row[] = '<td>
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton2" style="">
                <a class="dropdown-item edit-user" onclick="edit_user('.$template->id.')" user_id="'.$template->id.'" ><i class="bx bxs-pencil mr-2"></i> Edit Profile</a>
                <a class="dropdown-item text-danger" onclick="remove_user_info('.$template->id.')"><i class="bx bxs-trash mr-2"></i> Remove</a>
            </div>
        </td>';
  ...
  public function update_adminuser()
  {
    ...
    $params['role'] = 1;
    $accesscheck = !isset($params['accesscheck'])?[]:$params['accesscheck'];
    if (!empty($params['id']))
    {
      $user_id = $params['id'];
      unset($params['id']);
  ...
  // Job Post by Vadim
  public function job_post($value = '')
  {
    $this->common_model->checkAdminUserPermission(1);
    $this->data['page'] = 'job_post';
    $this->load->vars($this->data);
    $this->load->view($this->data['theme'] . '/template');
  }
