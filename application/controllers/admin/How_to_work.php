...
/**
 * @author: Leo 
 * @desc: How to work CRUD on super admin panel
 * @created: 
*/
...
class How_to_work extends CI_Controller
{
    ...
    public function index($offset = 0)
    {
        ...
        $dataList = $this->HowToWorkModel->List();
    ...
    public function create()
    {
        ...
        if ($this->input->post('form_submit')) {
            if ($this->data['admin_id'] > 1) {
                ...
                redirect(base_url() . 'admin/how_to_work');
            } else {
                $data = array();
                ...
                if (isset($_FILES) && isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])) {
                    ...
                    $upload_sts = $this->common->global_file_upload('uploads/how_to_work_images/', 'image', time() . $filename);
                    
                    if (isset($upload_sts['success']) && $upload_sts['success'] == 'y') {
                        $uploaded_file_name = $upload_sts['data']['file_name'];
    
                        if (!empty($uploaded_file_name)) {
                            $image_url = 'uploads/how_to_work_images/' . $uploaded_file_name;
                            // $data['image'] = image_resize(480, 320, $image_url,  $uploaded_file_name,"how_to_work_images");
                ...
                if ($this->HowToWorkModel->add($data)) {
    ...
    {
        $this->data['page'] = 'edit';
        $datalist = $this->HowToWorkModel->get($id);
        ...
        if ($this->data['admin_id'] > 1) {
            ...
            if ($this->input->post('form_submit')) {
                ...
                if ($this->HowToWorkModel->update($id, $data)) {
