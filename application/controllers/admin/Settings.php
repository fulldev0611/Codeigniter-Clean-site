...
class Settings extends CI_Controller
{
    ...
    public function otp_verify()
    {
        ...
        // $phone = 07488233413;
        // $numbers = array($phone);  //phone number
    ...
    {
        // $sess_mobil_con=$this->session->userdata('verify_otp');
        // if ($sess_mobil_con=="verified") {
        ...
        if ($this->input->post('form_submit'))
        {
            ...
            // changed by MaksimU for delete commission setting =============================
            // $admin_id = $this->session->userdata('admin_id');
            // $commission = $this->input->post('commission');
            // $CommInsert = ['admin_id' => $admin_id, 'commission' => $commission, ];
            ...
            if ($data)
            {
                ...
                if (!isset($data["host_seller_site"])) {
                    $data["host_seller_site"] = "off";
                }
                ...
                foreach ($data as $key => $val)
                {
                    if ($key != 'form_submit')
                    {
                        ...
                        $table_data['value'] = $val;
                        $this->db->where('key', $key);
                        if ($this->db->get("system_settings")->num_rows() > 0) {
                            $this->db->reset_query(); 
                            $this->db->where('key', $key);
        ...
        $this->load->view($this->data['theme'] . '/template');
        // }else{
        //     redirect('admin/settings_verifyotp');
        // }
    ...
    public function otp_verify_email()
    {
        ...
        // userdata :
        //set_userdata  : seesion ka naam aur uski value
        //unset_userdata
        

        if ($mobile_otp == $sess_mobil)
        {
            $redirect_con = $this->session->set_userdata('verify_otp', 'verified');
            redirect('admin/settings');
        }
        else
        {
            redirect('admin/settings_verifyotp?msg=otp_not_verify');
    ...
    public function stripe_payment_gateway()
    {
        ...
        if ($this->input->post('form_submit'))
        {
            ...
            $data['rest_key'] = $this->input->post('rest_key');
            $data['status'] = '1';
            $this->db->where('id', $id);
            if ($this->db->update('payment_gateways', $data))
            {
                if ($this->input->post('gateway_type') == 'test')
                {
                    $datas['publishable_key'] = $this->input->post('api_key');
                    $datas['secret_key'] = $this->input->post('secret_key');
                    $datas['rest_key'] = $this->input->post('rest_key');
                }
                else
                {
                    $datas['live_publishable_key'] = $this->input->post('api_key');
                    $datas['live_secret_key'] = $this->input->post('secret_key');
                    $datas['live_rest_key'] = $this->input->post('rest_key');
                }
        ...
        if ($this->input->post('form_submit'))
        {
            ...
            if ($this->db->update('paypal_payment_gateways', $data))
            {
                if ($this->input->post('paypal_gateway') == 'sandbox')
                {
                    ...
                    $datass['paypal_client_id'] = $this->input->post('client_id');
                    $datass['paypal_secret'] = $this->input->post('secret');
                }
                else
                {
                    $datass['braintree_key'] = $this->input->post('braintree_key');
