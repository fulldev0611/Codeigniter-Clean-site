...
class Service extends CI_Controller
{
  ...
  public function subscriptions()
  {
    ...
    if ($this->session->userdata('admin_id'))
    {
      ...
      $this->data['model'] = 'service';
      $this->data['list'] = $this->service->all_subscription_list();
  ...
  public function employee_form()
  {
    ...
    if ($this->session->userdata('admin_id'))
    {
      ...
      $this->data['employees'] = $this->Employee->getList(); 
      $this->load->vars($this->data);
      $this->load->view($this->data['theme'] . '/template');
    }
    else
  ...
  public function update_employee_status(){
    ...
    $this->load->model("Employee");
    $result = $this->Employee->update_status($data); 
    if($result){
  ...
  public function add_employee(){
      ...
      $edit_employee_id = $this->input->post('employee_id');
      $edit_user_id = $this->input->post('user_id');

      $this->load->model("Employee");
      $this->load->model('user_login_model', 'user_login');

      if(empty($edit_employee_id) && empty($edit_employee_id))
      {
          ...
          $ret = $this->Employee->insertEmployee($input_data);  
      } else{
          $username = strlen($input_data['name']);
          $input_data['share_code'] = $this->user_login->ShareCode(6, $username);
          $input_data['currency_code'] = settings('currency');
          ...
          $this->Employee->updateEmployeeDetail($input_data);  
          $ret = 'ok';
      }
  ...
  /* Leo */
  public function ajax_employee_info()
  {
    $input_data = $this->input->post();
    $employee_id = $input_data['employee_id'];
    $this->load->model("Employee");
    $data = $this->Employee->getDetail($employee_id); 
  ...
  public function ajax_remove_employee()
  {
      ...
      $this->load->model("Employee");
      $res = $this->Employee->deleteEmployee($employee_id);
  ...
  public function update_service()
  {
    ...
    $service_offered = json_encode(array(
      $service
    ));
    ...
    $inputs['measurement'] = $this->input->post('measurement');   // Leo
    ...
    $result = $this->service->update_service($inputs,['id'=>$_POST['service_id']]);
    /*---------------------------------------------------*/
  ...
  public function service_list()
  {
    ...
    if ($this->input->post('form_submit'))
    {
      ...
      $from = $this->input->post('from');
      $to = $this->input->post('to');
      $this->data['list'] = $this->service->service_filter($service_title, $category, $subcategory, $from, $to, null);
    }
    else
    {
      $this->data['list'] = $this->service->service_list(null);
    }
    $this->load->vars($this->data);
    $this->load->view($this->data['theme'] . '/template');
  ...
  public function change_Status_service_list()
  {
    ...
    if ($status == 0)
    {
      ...
      {
        $this->db->where('id', $id);
        if ($this->db->update('services', array(
          'status' => $status
        )))
        {
          echo "success";
