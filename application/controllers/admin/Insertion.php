...
class Insertion extends CI_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->input->post('form_submit')) {
            if ($this->data['admin_id'] > 1) {
                ...
                redirect(base_url() . 'admin/insertion');
            } else {
                $data = array();
                $data['commission'] = $this->input->post('commission');
                if ($this->InsertionModel->add($data)) {
    ...
    {
        $this->data['page'] = 'edit';
        $datalist = $this->InsertionModel->get($id);
        $this->data['datalist'] = $datalist;
        if ($this->data['admin_id'] > 1) {
            $this->session->set_flashdata('message', '<p class="alert alert-danger">Permission Denied</p>');
            redirect(base_url() . 'insertion');
        } else {
            if ($this->input->post('form_submit')) {
                $data = array();
                $data['commission'] = $this->input->post('commission');
                ...
                if ($this->InsertionModel->update($id, $data)) {
