...
class Subscription extends MY_Controller {  # upgrade maksimU for whitelabel using MY_Controller
   ...
   public function __construct() {
        ...
        $live_secret_key = $result['live_secret_key'];
        $live_publishable_key = $result['live_publishable_key'];
        $stripe_option = $result['stripe_option'];
        $this->data['website_logo_front'] = $result['logo_front'];
  ...
  public function withdraw_subscription() {
    $token=$this->session->userdata('chat_token');
    $this->send_push_notification($token,$this->session->userdata('id'),1,'(withdraw subscription): ' . $this->input->post('withdraw_message'));
    echo json_encode('successfully received'); exit;
  }
  ...
  public function send_push_notification($token,$provider_id,$type,$msg=''){
    ...
    if(!empty($data)){
      ...
      $receiver='';
      $subscription_module_id = $this->db->select('id')->from('admin_modules')->where('module_name','Subscriptions')->get()->row_array()['id'];
      //  print_r($subscription_module_id);exit;
      $admin_ids = $this->db->select('admin_id')->from('admin_access')->where('module_id', $subscription_module_id)->where('access', 1)->where_not_in('admin_id', array(1))->get()->result_array();
      foreach($admin_ids as $k=>$admin_id) {
        $admin_ids[$k] = $admin_id['admin_id'];
      }
      // print_r($admin_ids);exit;            
      $admins=$this->db->where_in('user_id',$admin_ids)->from('administrators')->get()->result_array();
      // print_r($admins);exit;
