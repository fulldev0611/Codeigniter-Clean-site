...
class Service extends MY_Controller     # upgrade maksimU for whitelabel using MY_Controller
{
    ...
    public function mail($value = '')
    {
        ...
        $result = $this->email->from('info@tazzer.com')->reply_to('info@tazzer.com') // Optional, an account where a human being reads.
        ->to('info@tazzer.com')->subject('hai')->message('asf')->send();
    }
    ...
    public function add_service()
    {
        ...
        if ($this->input->post('form_submit'))
        {
            ...
            /* Leo: use settingValue() common func */
            ...
            // $inputs['cat_id'] = $this->input->post('cat_id');
            // $inputs['dimension'] = $this->input->post('dimension');
    ...
    */
    ...
    {
        checkLoginRedirect();  // Leo
        checkIfNotProvider();  // Leo

        // get user full name from session user id     
        $userId = $this->session->userdata('id');        
        $query = $this->db->query('SELECT CONCAT(NAME,"  " ,l_name)  full_name  FROM users  WHERE id = '.$userId); 
        $result = $query->result();
        $row = $result['0'];
    ...
    /* # delete by maksimU : maksimU moving to employee
